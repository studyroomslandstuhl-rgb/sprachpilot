...
function questions(task,day){
  ...
  if(task==='change')return active.map(verb=>({id:`change-${verb.v}`,verb,p:`Zu welcher Gruppe gehört „${verb.v}“?`,a:changeGroupLabel(verb),o:CHANGES,hideChange:true}));
...
function renderQuiz(){
  ...
  const questionLabel=question.hideVerb?'Wortschatz · Wiederholung':question.hideChange?`${question.verb.v} · Gruppe finden`:`${question.verb.v} · ${question.verb.change}`;

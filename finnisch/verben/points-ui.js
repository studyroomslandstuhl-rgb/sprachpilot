...
function isPreview(){const r=String(localStorage.getItem('SP_LOGIN_ROLE')||localStorage.getItem('SP_ACTIVE_ROLE')||'').toLowerCase();return r==='teacher'||r==='lehrer'||r==='admin'||r==='owner'}
...
function write(s){if(isPreview())return;try{localStorage.setItem(key(),JSON.stringify(s))}catch{}}
...
function startNext(id){if(isPreview())return false;const state=read(),c=current(id,state);if(!c||!canRepeat(id,state))return false;const next=c.n+1;c.gs.currentRun=next;c.gs.runs=c.gs.runs||{};c.gs.runs[String(next)]={tasks:{},awards:{tasks:{},examPoints:0},exam:{bestPercent:0,stars:0,session:null}};write(state);return true}
...
function decorate(){
 if(!location.pathname.startsWith('/finnisch/verben/')||isPreview())return;

(function(){
  ...
  function header(){
    ...
    topbar.innerHTML='<div class="topbar-main"><a class="brand" href="/index.html"><img src="/assets/logo/sprachpilot-logo.png" alt=""><div><h1>SprachPilot</h1><div class="subtitle">Verben Test</div></div></a><div class="account"><span class="pill">'+esc(name)+' · '+esc(kurs)+'</span><a class="btn secondary" href="/student-dashboard/index.html">Dashboard</a><a class="btn secondary" href="/profile/index.html">Profil</a></div></div><nav class="nav"><button class="secondary" data-go="home">← Zurück</button><button class="secondary" data-go="overview">Aufgabenübersicht</button><button class="danger" data-action="reset">Fortschritte löschen</button></nav>';
  }
  ...
  function renderHome(){var p=state.package;app.innerHTML='<section class="card hero"><h2>Was möchtest du machen?</h2><div class="actions"><button '+(p?'':'disabled')+' data-go="overview">Üben</button><button data-go="assess">Verben einschätzen</button><button data-go="choose">Verben wählen</button></div><p class="notice">'+(p?'Aktuelles Paket: '+p.verbs.length+' Verben · Runde '+p.round:'Bitte zuerst Verben wählen oder einschätzen.')+'</p></section><section class="card"><div class="stats"><div class="stat"><b>'+catalog.length+'</b>mögliche Verben</div><div class="stat"><b>'+state.learned.length+'</b>gelernt</div><div class="stat"><b>'+(p?p.verbs.length:state.active.length)+'</b>zum Üben</div><div class="stat"><b>'+state.points+'</b>Punkte</div></div></section>'+(state.lastComplete?'<section class="card finish"><h2>Gratulation!</h2><p>'+esc(state.lastComplete)+'</p><div class="actions"><button data-go="choose">Neue Verben wählen</button><button class="secondary" data-go="assess">Verben einschätzen</button></div></section>':'');}
  ...
  function renderAssessmentBlocked(message){app.innerHTML='<section class="card"><h2>Neue Verben einschätzen</h2><p class="small">'+esc(message)+'</p><div class="actions"><button data-go="overview">Aufgaben starten</button><button class="secondary" data-go="home">Zur Verben-Seite</button></div></section>';}
  function renderAssess(){
    ...
    if(!v){if(state.active.length){startPackage(state.active);return;}app.innerHTML='<section class="card finish"><h2>Du hast alle Verben eingeschätzt.</h2><p class="small">Es gibt keine weiteren offenen Verben.</p><button data-go="home">Zur Verben-Seite</button></section>';return;}
  ...
  function renderOverview(){
    ...
    var exam='<button class="module '+(allTasksDone()?'':'locked')+'" data-go="exam"><div class="icon">⭐</div><h3>Prüfung</h3><b>'+(allTasksDone()?(p.exam.best||0)+'%':'gesperrt')+'</b></button>';
  ...
  function taskShell(id,body,feedback){app.innerHTML='<section class="card"><div class="task-head"><div><h2>'+esc(taskTitle(id))+'</h2>'+taskProgress(id)+'</div><button class="secondary" data-go="overview">Aufgabenübersicht</button></div>'+body+'<div class="feedback">'+(feedback||'')+'</div></section>';}
  ...
  function renderExam(){if(!state.package){go('choose');return;}if(!allTasksDone()){app.innerHTML='<section class="card locked-box"><h2>Prüfung gesperrt</h2><p>Bearbeite zuerst alle Aufgaben zu 100%.</p><button data-go="overview">Zurück</button></section>';return;}var p=state.package;if(!p.exam.run)p.exam.run={index:0,answers:[],verbs:shuffle(p.verbs)};var run=p.exam.run,v=run.verbs[run.index];if(!v)return finishExam();app.innerHTML='<section class="card"><div class="task-head"><div><h2>⭐ Prüfung</h2><p>Frage '+(run.index+1)+' von '+run.verbs.length+'</p></div><button class="secondary" data-go="overview">Aufgabenübersicht</button></div>'+imgHtml(v)+'<div class="question">Schreibe das Verb.</div><input class="answer-input" data-input placeholder="Verb schreiben"><div class="actions"><button data-exam-check="'+esc(v)+'">Kontrollieren</button></div><div class="feedback"></div></section>';save();}
  function finishExam(){var p=state.package,run=p.exam.run,right=run.answers.filter(function(a){return a.right;}).length,score=Math.round(right*100/p.verbs.length);p.exam.best=Math.max(p.exam.best||0,score);if(score>=100&&!p.exam.pointsGiven){p.exam.pointsGiven=true;p.points+=pointForExam();state.points+=pointForExam();}var learnedNow=[];if(score>=100){learnedNow=p.verbs.slice();state.learned=uniq(state.learned.concat(learnedNow));state.known=uniq(state.known.concat(learnedNow));state.active=[];state.lastComplete=learnedNow.length+' Verben wurden als gelernt gespeichert.';state.package=null;save();app.innerHTML='<section class="card finish"><h2>Gratulation!</h2><p>Prüfung: 100%. Die Verben sind als gelernt gespeichert.</p><div class="actions"><button data-go="choose">Neue Verben wählen</button><button class="secondary" data-go="assess">Verben einschätzen</button></div></section>';return;}p.exam.run=null;save();app.innerHTML='<section class="card finish"><h2>Prüfung beendet</h2><p>'+score+'% richtig. Die Prüfung kann wiederholt werden.</p><div class="actions"><button data-go="exam">Prüfung wiederholen</button><button class="secondary" data-go="overview">Aufgabenübersicht</button></div></section>';}

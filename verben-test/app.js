(function(){
  ...
  function header(){
    ...
    var back=backTarget();
    topbar.innerHTML='<div class="topbar-main"><a class="brand" href="/index.html"><img src="/assets/logo/sprachpilot-logo.png" alt=""><div><h1>SprachPilot</h1><div class="subtitle">Verben Test</div></div></a><div class="account"><span class="pill">'+esc(name)+' · '+esc(kurs)+'</span><a class="btn secondary" href="/student-dashboard/index.html">Dashboard</a><a class="btn secondary" href="/profile/index.html">Profil</a></div></div><nav class="nav"><a class="btn secondary" href="'+(back?routeHref(back):'/index.html')+'" data-back="'+(back||'')+'">← Zurück</a><a class="btn secondary" href="'+routeHref('overview')+'" data-go="overview">Übersicht</a><a class="btn secondary" href="'+routeHref('tasks')+'" data-go="tasks">Aufgabenübersicht</a><button class="danger" data-action="reset">Fortschritte löschen</button></nav>';
  }
  ...
  function routeHref(route){
    ...
    if(route==='tasks')return '/verben-test/?view=tasks';
  ...
  function backTarget(){if(view==='task'||view==='exam')return 'tasks';if(view==='overview')return 'tasks';if(view==='tasks'||view==='choose'||view==='assess')return 'home';return '';}
  function routeFromUrl(){
    ...
    if(v==='overview'||v==='uebersicht')return {view:'overview',params:{}};
    if(v==='tasks'||v==='aufgaben')return {view:'tasks',params:{}};
  ...
  function renderAssessmentBlocked(message){app.innerHTML='<section class="card"><h2>Neue Verben einschätzen</h2><p class="small">'+esc(message)+'</p><div class="actions"><a class="btn" href="'+routeHref('tasks')+'" data-go="tasks">Aufgaben starten</a><a class="btn secondary" href="'+routeHref('home')+'" data-go="home">Zur Verben-Seite</a></div></section>';}
  ...
  function startPackage(list){
    ...
    state.package=normalizePackage(state.package);state.active=list.slice();state.learnList=[];state.lastComplete=null;state.currentAssessment='';save();go('tasks');
  }
  function renderVerbOverview(){
    if(!state.package){go('choose');return;}
    var p=state.package;
    var cards=p.verbs.map(function(v,i){var b=byVerb[v]||{};return '<div class="module"><div class="icon">'+(i+1)+'</div>'+imgHtml(v)+'<h3>'+esc(v)+'</h3><p>'+esc(b.translation||'')+'</p></div>';}).join('');
    app.innerHTML='<section class="card"><div class="task-head"><div><h2>Übersicht</h2><p>'+p.verbs.length+' Verben in diesem Paket.</p></div><a class="btn secondary" href="'+routeHref('tasks')+'" data-go="tasks">Aufgabenübersicht</a></div></section><section class="grid">'+cards+'</section>';
  }
  ...
  function taskShell(id,body,feedback){app.innerHTML='<section class="card"><div class="task-head"><div><h2>'+esc(taskTitle(id))+'</h2>'+taskProgress(id)+'</div><div class="actions"><a class="btn secondary" href="'+routeHref('overview')+'" data-go="overview">Übersicht</a><a class="btn secondary" href="'+routeHref('tasks')+'" data-go="tasks">Aufgabenübersicht</a></div></div>'+body+'<div class="feedback">'+(feedback||'')+'</div></section>';}
  function currentVerb(id){var v=nextVerb(id);if(!v){app.innerHTML='<section class="card finish"><h2>Gut gemacht</h2><p>Diese Aufgabe ist abgeschlossen.</p><a class="btn" href="'+routeHref('tasks')+'" data-go="tasks">Weiter</a></section>';return null;}return v;}
  function renderTask(id){if(!state.package){go('choose');return;}if(TASK_IDS.indexOf(id)<0){go('tasks');return;}if(id==='karteikarten')return renderFlashcards();if(id==='bild-verb')return renderImageToVerb();if(id==='verb-bild')return renderVerbToImage();if(id==='hoeren-schreiben')return renderListenWrite();if(id==='schreiben')return renderWrite();if(id==='satz-bauen')return renderSentenceBuild();if(id==='konjugieren')return renderConjugation();}

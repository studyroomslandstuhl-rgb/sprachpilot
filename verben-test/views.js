...
function examCard(){const open=allTasksComplete(),best=activePackage()?.examBest||0;return `<a class="module ${open?'':'locked'}" href="${open?'?task=pruefung':'#'}" ${open?'data-link-route':'aria-disabled="true"'}><div class="icon">⭐</div><h3>Prüfung</h3><p class="small">Eine zufällige Frage pro Paketverb. Das beste Ergebnis zählt.</p><div class="progress"><div class="bar" style="width:${best}%"></div></div><div class="small">${open?`${best}/100 Punkte`:'Erst nach allen Aufgaben'}</div></a>`}
function bindRouteButtons(){
  document.querySelectorAll('[data-route]').forEach(btn=>{btn.onclick=()=>{const target=btn.dataset.route;if(target==='home')navigate({});else if(target==='choose')navigate({mode:'choose'});else if(target==='assess')navigate({mode:'assess'});else if(target==='groups')navigate({view:'groups'})}});
  document.querySelectorAll('[data-link-route]').forEach(link=>{link.onclick=e=>{e.preventDefault();const url=new URL(link.href,location.href);navigate(Object.fromEntries(url.searchParams.entries()))}});
}
...
function renderGroups(){
  ...
  document.querySelectorAll('[data-group-index]').forEach(btn=>btn.onclick=()=>draw(Number(btn.dataset.groupIndex)));bindRouteButtons();draw(0);
}
function renderChoose(restoreFocus=false){
  ...
  search.oninput=e=>{choiceSearch=e.target.value;renderChoose(true)};
  document.querySelectorAll('.verb-row input').forEach(input=>input.onchange=()=>{if(input.checked){if(choiceSelection.size<required)choiceSelection.add(input.value);else input.checked=false}else choiceSelection.delete(input.value);renderChoose()});
...
function renderAssessment(){
  ...
  document.getElementById('assessmentLearned').onclick=()=>{state.learned=uniq([...state.learned,verb]);a.index++;persistLocal();renderAssessment()};
  document.getElementById('assessmentActive').onclick=()=>{a.selected=uniq([...a.selected,verb]);a.index++;persistLocal();renderAssessment()};bindRouteButtons();
}
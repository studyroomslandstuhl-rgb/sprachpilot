...
assert.ok(src.includes('committed=true'),'coordinated update must mark Firestore commit before post-commit session cleanup');
assert.ok(src.includes('if(!committed&&authChanged'),'Firebase Auth rollback must never run after Firestore already committed the new email');
...
assert.ok(src.includes("updateUser(authUid,{email:oldAuthEmail,emailVerified:oldVerified})"),'Firebase Auth change must roll back when the coordinated update fails before commit');

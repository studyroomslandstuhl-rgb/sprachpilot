...
async function sync(){
 ...
 try{
  ...
  const perfekt={...(progress.perfekt||{})},actual=modulePoints(perfekt,CARRY_ID),target=Math.max(localPerfektTarget(),actual),carryNeeded=Math.max(0,target-actual),oldCarry=perfekt[CARRY_ID]||{};
  ...
  const nowIso=new Date().toISOString(),metadata={...(progress.metadata||{}),perfektGroups:groupMetadata()},next={...progress,perfekt,metadata},rankingPoints=allModulePoints(next),c=course(p);
  ...
  window.SP_PERFEKT_FIREBASE_SYNC={ok:true,perfektPoints:actual+carryNeeded,target,rankingPoints,at:nowIso}

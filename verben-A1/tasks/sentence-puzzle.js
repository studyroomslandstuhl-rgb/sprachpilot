...
function sentencePuzzle(){
  ...
  puzzleTarget=sentenceForVerb(v);const words=shuffle(puzzleTarget.split(/\s+/));
...
function resetPuzzleWords(){const v=state.currentTask&&state.currentTask.v;if(!v)return;const words=shuffle(puzzleTarget.split(/\s+/));$("puzzleAnswer").innerHTML="";$("puzzleBank").innerHTML=words.map(w=>`<button class="word-chip" onclick="movePuzzleWord(this)">${safeText(w)}</button>`).join("")}

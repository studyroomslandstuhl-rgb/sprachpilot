...
function renderVerbExam(){
  ...
  if(item.type==="conjugate")body=`${imageBox(item.v)}<p class="small">Schreibe die richtige Form.</p><div class="assessment-card"><div class="german-word sentence-gap">${safeText(conjugationSentence(item.v,item.subj))}</div></div><input id="examInput" placeholder="Verbform schreiben">`;
...
function checkVerbExamAnswer(){
  ...
  ex.answers.push({type:item.type,v:item.v,good,answer,solution:sol});
  if(good){$("fb").innerHTML="<div class='ok'>Richtig.</div>"}else{$("fb").innerHTML=`<div class='no'>Falsch. Lösung: <strong>${safeText(sol)}</strong></div>`}
  ex.current+=1;saveState();setTimeout(renderVerbExam,good?600:1200);
}

...
function queuedProgress(skill){const dKey=taskDoneSetKey(skill);state.practicePool=releaseFilterVerbs(state.practicePool||[]);state.taskDoneSets[dKey]=releaseFilterVerbs((state.taskDoneSets[dKey]||[]).map(k=>String(k).split(":")[0])).map(v=>v+":0");const done=(state.taskDoneSets[dKey]||[]).length;const total=((state.practicePool&&state.practicePool.length)?releaseFilterVerbs(state.practicePool):currentPracticeVerbs()).length;return {done:Math.min(done,total),total,pct:total?Math.min(100,Math.round(Math.min(done,total)*100/total)):0}}

...
function sendProgress(){if(typeof spCanWriteFirebaseProgress==="function"&&!spCanWriteFirebaseProgress())return;if(typeof canSaveVerbProgress==="function"&&!canSaveVerbProgress())return;if(!profile||!db)return;const sid=firebaseStudentId();if(!sid)return;clearTimeout(firebaseSaveTimer);firebaseSaveTimer=setTimeout(async()=>{try{if(typeof spCanWriteFirebaseProgress==="function"&&!spCanWriteFirebaseProgress())return;const progressValue=overall();await db.collection("progress").doc(sid).set({studentId:sid,kurs:profile.kurs||profile.kursnummer||"",verben:{progress:progressValue,stars:totalStars(),activeVerbs:state.active||[],learnedVerbs:state.learned||[],known:state.known||[],unsure:state.unsure||[],unknown:state.unknown||[],assessed:state.assessed||[],currentPackageVerbs:currentPackageAllVerbs(),exam:state.exam||{},state:state,updatedAt:firebase.firestore.FieldValue.serverTimestamp()},updatedAt:firebase.firestore.FieldValue.serverTimestamp()},{merge:true});await db.collection("students").doc(sid).set({studentId:sid,vorname:profile.vorname||"",nachname:profile.nachname||"",kurs:profile.kurs||profile.kursnummer||"",muttersprache:profile.muttersprache||"",verbenFortschritt:progressValue,lastActivity:firebase.firestore.FieldValue.serverTimestamp()},{merge:true})}catch(e){console.warn("Firebase Speichern fehlgeschlagen",e)}},600)}
...
function currentTaskItem(skill){
  ...
  if(state.currentTask && state.currentTask.skill===sk && state.currentTask.v)return state.currentTask;
  initTaskQueue(sk);
...
function finishQueuedVerb(skill,v,good=true){
  ...
  if(good){
    ...
    else {const source=(state.practicePool&&state.practicePool.length)?state.practicePool:buildPracticePool();const found=source.findIndex((x,i)=>x===v&&!state.taskDoneSets[dKey].includes(x+":"+i));if(found>=0)state.taskDoneSets[dKey].push(v+":"+found)}
  } else {

...
function spGoBack(){if(isTaskPhase()){closeOpenVerbTask();goVerbIndex();return}if(state.phase==='overview'||state.phase==='chooser'||state.phase==='home'){goVerbIndex();return}goStudentDashboard()}
function closeVerbTaskAndRenderHome(){closeOpenVerbTask();goVerbIndex()}
...
function openNextTask(){for(const s of VERB_SKILLS){if(!taskDone(s)){openVerbTask(s);return}}renderVerbIndexPage()}
...
function renderVerbIndexPage(){
  ...
  const remainingCount=released.length-learnedCount-active.length;
  const canPractice=active.length>0;
  app.classList.remove('card');
  app.innerHTML=`<section class="card verb-index-card"><p class="eyebrow">Verben A1</p><h2>Was möchtest du machen?</h2><p class="small">${released.length} freigegebene Verben · ${learnedCount} gelernt · ${active.length} zum Üben · ${Math.max(0,remainingCount)} noch offen</p><div class="grid verb-start-grid"><button type="button" class="module task-card ${canPractice?'':'disabled-card'}" ${canPractice?'onclick="renderTaskOverview()"':'disabled'}><div class="icon big-icon">▶</div><div class="num">Üben</div><p>${canPractice?'Aufgaben mit deinen unsicheren Verben starten.':'Erst Verben einschätzen oder wählen.'}</p><div class="start">${canPractice?'Starten':'Noch gesperrt'}</div></button><button type="button" class="module task-card" onclick="handleAssessmentClick()"><div class="icon big-icon">?</div><div class="num">Einschätzen</div><p>Neue Verben prüfen: ich kann / unsicher / ich kann nicht.</p><div class="start">Starten</div></button><button type="button" class="module task-card" onclick="renderVerbChooser()"><div class="icon big-icon">✓</div><div class="num">Wählen</div><p>Freigegebene, noch nicht gelernte Verben selbst auswählen.</p><div class="start">Auswählen</div></button></div></section>`;
}
...
function renderSafeHomeFallback(error){try{localStorage.setItem('SP_VERBS_LAST_BOOT_ERROR',String(error&&error.stack||error||'unknown'))}catch(e){}try{renderVerbIndexPage()}catch(e){try{renderVerbChooser()}catch(x){goStudentDashboard()}}}
async function boot(){if(!loadProfile())return;renderHeader();await loadState();spEnsureState();saveState();renderHeader();renderSideMenu();renderVerbIndexPage();spSyncDashboardSummary();renderAndHydrate()}
window.addEventListener('hashchange',()=>{if(!profile)return;const hp=phaseFromHash();if(hp==='overview'){renderVerbOverview();return}if(hp==='chooser'){renderVerbChooser();return}if(hp==='assessment'){try{renderAssessment(true);return}catch(e){renderVerbIndexPage();return}}renderVerbIndexPage()});

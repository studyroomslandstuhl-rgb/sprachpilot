...
function goTaskOverview(){clearHash();if(currentPracticeVerbs().length){renderTaskOverview();return}renderHome()}
function spGoBack(){if(isTaskPhase()){closeOpenVerbTask();goTaskOverview();return}if(state.phase==='overview'||state.phase==='chooser'||state.phase==='home'){goTaskOverview();return}goStudentDashboard()}
...
function renderTaskOverview(){
  ...
  spEnsureState();
  if(packageExamPassed()&&allPracticeTasksDone()&&currentPracticeVerbs().length){markCurrentPackageLearned();renderHome();return}
  const active=currentPracticeVerbs();
  if(!active.length){renderHome();return}
  state.phase='home';state.currentTask=null;
  if(!state.practicePool.length)buildPracticePool();
  saveState();spSyncDashboardSummary();clearHash();
...
function handleAssessmentClick(){spEnsureState();if(currentPracticeVerbs().length){renderTaskOverview();return}try{startAssessment(true)}catch(e){console.error('Assessment failed',e);renderVerbChooser()}}
function renderAllVerbsCompletedPage(){const app=$('app');if(!app)return;app.classList.remove('card');state.phase='home';saveState();spSyncDashboardSummary();app.innerHTML='<section class="card completion-card"><div class="finish-icon">✓</div><h2>Du hast alle Verben gelernt.</h2><p class="small">Komm später zurück, um neue Verben zu lernen.</p></section>'}
...
function renderHome(){
  ...
  if(!releasedVerbList().length){renderNoReleasedPage();return}
...
function renderSafeHomeFallback(error){try{localStorage.setItem('SP_VERBS_LAST_BOOT_ERROR',String(error&&error.stack||error||'unknown'))}catch(e){}try{renderHome()}catch(e){try{renderVerbChooser()}catch(x){goStudentDashboard()}}}
...
window.addEventListener('hashchange',()=>{if(!profile)return;const hp=phaseFromHash();if(hp==='overview'){renderVerbOverview();return}if(hp==='chooser'){renderVerbChooser();return}if(hp==='assessment'){try{if(currentPracticeVerbs().length){renderTaskOverview();return}renderAssessment(true);return}catch(e){renderHome();return}}renderHome()});

...
function spEnsureState(){
  ...
  state.active=restrict([...(state.active||[]),...(state.unsure||[]),...(state.unknown||[]),...(state.currentPackageVerbs||[]),...(state.assessmentBatch||[])]).slice(0,MAX_PRACTICE_VERBS);
  ...
  state.practicePool=restrict(state.practicePool||[]).filter(v=>state.active.includes(v)).slice(0,MAX_PRACTICE_VERBS);
  state.active.forEach(ensureSkillState);
}
function currentPracticeVerbs(){spEnsureState();return state.active.slice(0,MAX_PRACTICE_VERBS)}
function currentPackageAllVerbs(){spEnsureState();return spUniq([...(state.currentPackageVerbs||[]),...(state.assessmentBatch||[]),...(state.active||[])]).slice(0,MAX_PRACTICE_VERBS)}
...
window.spVerbPracticeTargetCount=()=>{const n=remainingUnlearnedVerbs().length+currentPracticeVerbs().length;return Math.min(MAX_PRACTICE_VERBS,n||MAX_PRACTICE_VERBS)};
...
function setVerbenUrl(query,replace=false){const url='/verben-A1/'+(query?('?'+query):'');try{(replace?history.replaceState:history.pushState).call(history,null,'',url)}catch(e){location.href=url}}
...
function goVerbIndex(){setVerbenUrl('',false);renderVerbIndexPage()}
function goTaskOverview(){setVerbenUrl('view=aufgaben',false);renderTaskOverview()}
function goOverviewView(){setVerbenUrl('view=overview',false);renderVerbOverview()}
function goChooserView(){setVerbenUrl('view=chooser',false);renderVerbChooser()}
function goAssessmentView(){setVerbenUrl('view=assessment',false);handleAssessmentClick()}
function startPractice(){spEnsureState();if(currentPracticeVerbs().length){goTaskOverview();return}renderVerbIndexPage()}
function spGoBack(){if(isTaskPhase()){closeOpenVerbTask();goTaskOverview();return}if(state.phase==='taskOverview'){goVerbIndex();return}goVerbIndex()}
...
function taskSlug(skill){return PHASE_HASHES[skill]||skill}
function taskHref(skill){return '/verben-A1/?task='+encodeURIComponent(taskSlug(skill))}
...
function examCard(){const ready=allPracticeTasksDone(),passed=state.exam&&state.exam.passed;return `<a class="module task-card ${passed?'done-card':''} ${ready?'':'disabled-card'}" ${ready?'href="/verben-A1/?task=pruefung"':'aria-disabled="true"'}><div><div class="num">11. Prüfung</div></div><div class="icon big-icon">⭐</div><p>Prüfe die aktiven Verben.</p><div><div class="progress"><div class="bar" style="width:${passed?100:0}%"></div></div><div class="small">${passed?'100%':ready?'offen':'gesperrt'}</div><div class="start">Starten</div></div></a>`}
...
function renderVerbIndexPage(){
  ...
  app.classList.remove('card');
  app.innerHTML=`<section class="card verb-index-card"><p class="eyebrow">Verben A1</p><h2>Was möchtest du machen?</h2><p class="small verb-index-summary"><span>${released.length} freigegebene Verben</span><span>${learnedCount} gelernt</span><strong>${active.length} zum Üben</strong><span>${remainingCount} noch offen</span></p><div class="grid verb-start-grid">${canPractice?`<a class="module task-card verb-action-card verb-practice-card" href="/verben-A1/?view=aufgaben" onclick="startPractice();return false"><div class="verb-action-visual"><span class="verb-action-symbol">▶</span></div><div class="num">Üben</div><p>Aufgaben mit deinen unsicheren Verben starten.</p><div class="start">Starten</div></a>`:`<button type="button" class="module task-card verb-action-card disabled-card" disabled><div class="verb-action-visual"><span class="verb-action-symbol">▶</span></div><div class="num">Üben</div><p>Erst Verben einschätzen oder wählen.</p><div class="start">Noch gesperrt</div></button>`}<a class="module task-card verb-action-card" href="/verben-A1/?view=assessment"><div class="verb-action-visual"><span class="verb-action-symbol">?</span></div><div class="num">Einschätzen</div><p>Neue Verben prüfen: ich kann / unsicher / ich kann nicht.</p><div class="start">Starten</div></a><a class="module task-card verb-action-card" href="/verben-A1/?view=chooser"><div class="verb-action-visual"><span class="verb-action-symbol">✓</span></div><div class="num">Wählen</div><p>Freigegebene, noch nicht gelernte Verben selbst auswählen.</p><div class="start">Auswählen</div></a></div></section>`;
}
...
function maxManualVerbSelection(){return Math.max(0,MAX_PRACTICE_VERBS-currentPracticeVerbs().length)}
function renderVerbChooser(){const app=$('app');if(!app)return;spEnsureState();if(allReleasedVerbsLearned()){renderAllVerbsCompletedPage();return}state.phase='chooser';const mastered=spMasteredVerbSet();const activeSet=new Set(currentPracticeVerbs());const max=maxManualVerbSelection();const verbs=max>0?releasedVerbList().filter(v=>!mastered.has(v)&&!activeSet.has(v)):[];const selected=new Set();app.classList.add('card');app.innerHTML=`<section class="card"><h2>Verben wählen</h2><p class="small">Du kannst beliebig zwischen 1 und 20 Verben üben. Aktuell hast du ${activeSet.size} aktive Verben. Du kannst noch ${max} Verben auswählen. Gelernte Verben erscheinen hier nicht.</p><div class="actions"><input id="manualVerbSearch" oninput="spFilterManualVerbs()" placeholder="Verb suchen" style="max-width:320px"><span class="badge"><span id="manualVerbCount">${selected.size}</span>/${max} gewählt</span></div>${verbs.length?`<div class="verb-choice-grid" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:8px;margin-top:14px">${verbs.map(v=>`<button type="button" class="btn secondary" data-verb-choice="${safeText(v)}" style="text-align:left;white-space:normal"><b>${safeText(v)}</b></button>`).join('')}</div>`:`<div class="empty">Du hast bereits 20 aktive Verben oder keine weiteren freigegebenen Verben.</div>`}<div class="actions" style="margin-top:16px"><button class="btn green" onclick="spSaveManualVerbs()">Auswahl speichern</button><button class="btn secondary" onclick="goVerbIndex()">Zur Verben-Seite</button></div></section>`;saveState()}
...
function spToggleManualVerb(v){const box=[...document.querySelectorAll('[data-verb-choice]')].find(el=>el.getAttribute('data-verb-choice')===v);if(!box)return;const max=maxManualVerbSelection();const n=document.querySelectorAll('[data-verb-choice].selected').length;if(box.classList.contains('selected'))box.classList.remove('selected');else{if(n>=max){alert('Du kannst maximal 20 Verben gleichzeitig üben.');return}box.classList.add('selected')}const c=$('manualVerbCount');if(c)c.textContent=document.querySelectorAll('[data-verb-choice].selected').length}
function spSaveManualVerbs(){const allowed=spAllowedSet(),mastered=spMasteredVerbSet(),activeSet=new Set(currentPracticeVerbs()),max=maxManualVerbSelection();const chosen=[...document.querySelectorAll('[data-verb-choice].selected')].map(el=>el.getAttribute('data-verb-choice')).filter(v=>(!allowed.size||allowed.has(v))&&!mastered.has(v)&&!activeSet.has(v)).slice(0,max);if(!chosen.length){alert(max>0?'Bitte wähle mindestens ein freigegebenes Verb.':'Du hast bereits 20 aktive Verben.');return}const merged=spUniq([...currentPracticeVerbs(),...chosen]).slice(0,MAX_PRACTICE_VERBS);state.phase='home';state.active=merged.slice();state.unknown=spUniq([...(state.unknown||[]),...chosen]).filter(v=>merged.includes(v));state.unsure=spUniq(state.unsure||[]).filter(v=>merged.includes(v));state.currentPackageVerbs=merged.slice();state.assessmentBatch=merged.slice();state.practicePool=merged.slice();state.currentTask=null;state.taskQueues={};state.taskDoneSets={};state.memoryCards=[];state.memoryDone=[];state.openCards=[];state.exam={passed:false,score:0,stars:0,answers:[],current:0,items:[],awaiting:false,currentTry:0,hadWrong:false};saveState();spSyncDashboardSummary();goTaskOverview()}
...
function taskFromQuery(raw){const m={'bild-verb':'bild_verb','verb-bild':'verb_bild','hoeren-schreiben':'hoeren_schreiben','hoeren-sprechen':'hoeren_sprechen','bild-sprechen':'bild_sprechen','satz-puzzle':'satz_puzzle'};return m[raw]||raw}
function routeVerbenHash(){
  const params=new URLSearchParams(location.search||'');
  const view=params.get('view')||'';
  const task=taskFromQuery(params.get('task')||'');
  if(view==='aufgaben'){renderTaskOverview();return}
  if(view==='overview'){renderVerbOverview();return}
  if(view==='chooser'){renderVerbChooser();return}
  if(view==='assessment'){handleAssessmentClick();return}
  if(task){if(task==='pruefung'){if(allPracticeTasksDone())startVerbExam();else renderTaskOverview();return}if(VERB_SKILLS.includes(task)){openVerbTask(task);return}}
  const hp=phaseFromHash();if(hp==='taskOverview'){renderTaskOverview();return}if(VERB_SKILLS.includes(hp)){openVerbTask(hp);return}if(hp==='pruefung'){if(allPracticeTasksDone())startVerbExam();else renderTaskOverview();return}if(hp==='overview'){renderVerbOverview();return}if(hp==='chooser'){renderVerbChooser();return}if(hp==='assessment'){handleAssessmentClick();return}renderVerbIndexPage()
}
function renderSafeHomeFallback(error){try{localStorage.setItem('SP_VERBS_LAST_BOOT_ERROR',String(error&&error.stack||error||'unknown'))}catch(e){}try{routeVerbenHash()}catch(e){try{renderVerbIndexPage()}catch(x){goStudentDashboard()}}}
...
window.addEventListener('popstate',()=>{if(!profile)return;routeVerbenHash()});

...
// Keine Firebase-Schreibvorgänge, keine Release-Bridges, keine späten Overrides.
(function(){
  ...
  function mergeStates(a,b){a=a||{};b=b||{};const out={...a,...b};['known','learned','assessed','assessmentBatch','currentPackageVerbs','unsure','unknown','active','practicePool','memoryDone','openCards','archivedPackages'].forEach(k=>out[k]=union(a[k],b[k]));['weak','alertsShown','taskRewardsShown'].forEach(k=>out[k]=obj(a[k],b[k]));['skillDone','skillAttempts','skillSuccess','taskQueues','taskDoneSets'].forEach(k=>out[k]=deep(a[k],b[k]));out.exam=betterExam(a.exam,b.exam);out.manualVerbSelection=!!(a.manualVerbSelection||b.manualVerbSelection);return out}
  ...
  function safeClick(e){const b=e.target&&e.target.closest?e.target.closest('button,a'):null;if(!b)return;const action=b.dataset&&b.dataset.action;const txt=String(b.textContent||'').trim();const choice=b.getAttribute&&b.getAttribute('data-verb-choice');if(choice&&typeof window.spToggleManualVerb==='function'){e.preventDefault();e.stopPropagation();window.spToggleManualVerb(choice);return}if((action==='verb-chooser'||txt==='Verben wählen')&&typeof window.renderVerbChooser==='function'){e.preventDefault();e.stopPropagation();window.renderVerbChooser();return}if((action==='save-manual-verbs'||txt==='Auswahl speichern')&&typeof window.spSaveManualVerbs==='function'){e.preventDefault();e.stopPropagation();window.spSaveManualVerbs();return}if((action==='home'||txt==='Zurück')&&typeof window.closeVerbTaskAndRenderHome==='function'){e.preventDefault();e.stopPropagation();window.closeVerbTaskAndRenderHome();return}}

// data/a1-verb-expansion-2026-07.js
// Ergänzung: neue Verben mit sauberer Trennung nach trennbar, untrennbar, normal und irregulär.
(function(){
  const NEW_VERBS=[
  ...
  const TRANSLATIONS={
    'Englisch':{
      'vorhaben':'to plan / intend to do','planen':'to plan','aufgeben':'to give up','zuhören':'to listen','zusehen':'to watch','gehören':'to belong','abschreiben':'to copy','vorlesen':'to read aloud','verschlafen':'to oversleep / miss by sleeping'
    },
    'Russisch':{
      'vorhaben':'планировать / намереваться','planen':'планировать','aufgeben':'сдаваться / бросать','zuhören':'слушать внимательно','zusehen':'смотреть / наблюдать','gehören':'принадлежать','abschreiben':'списывать / переписывать','vorlesen':'читать вслух','verschlafen':'проспать'
    },
    'Ukrainisch':{
      'vorhaben':'планувати / мати намір','planen':'планувати','aufgeben':'здаватися / кидати','zuhören':'уважно слухати','zusehen':'дивитися / спостерігати','gehören':'належати','abschreiben':'списувати / переписувати','vorlesen':'читати вголос','verschlafen':'проспати'
    },
    'Arabisch':{
      'vorhaben':'ينوي / يخطط','planen':'يخطط','aufgeben':'يستسلم / يترك','zuhören':'يصغي','zusehen':'يشاهد','gehören':'ينتمي / يخص','abschreiben':'ينسخ','vorlesen':'يقرأ بصوت عالٍ','verschlafen':'ينام ويفوّت الموعد'
    },
    'Türkisch':{
      'vorhaben':'niyet etmek / planlamak','planen':'planlamak','aufgeben':'vazgeçmek','zuhören':'dinlemek','zusehen':'izlemek','gehören':'ait olmak','abschreiben':'kopyalamak / geçirmek','vorlesen':'sesli okumak','verschlafen':'uyuyakalmak / kaçırmak'
    },
    'Rumänisch':{
      'vorhaben':'a avea de gând','planen':'a planifica','aufgeben':'a renunța','zuhören':'a asculta atent','zusehen':'a privi','gehören':'a aparține','abschreiben':'a copia','vorlesen':'a citi cu voce tare','verschlafen':'a dormi prea mult / a rata din somn'
    },
    'Japanisch':{
      'vorhaben':'予定している','planen':'計画する','aufgeben':'あきらめる','zuhören':'よく聞く','zusehen':'見物する','gehören':'属する / 所有である','abschreiben':'書き写す','vorlesen':'音読する','verschlafen':'寝過ごす'
    }
  };
  ...
  const IRREGULAR=['vorhaben','aufgeben','zusehen','abschreiben','vorlesen','verschlafen'];
  const SEPARABLE=Object.keys(SEPARABLE_MAP);
  const INSEPARABLE=['verschlafen'];
  ...
  function addVerbs(){
    ...
    NEW_VERBS.forEach(item=>{if(!existing.has(item.v)){all.push(Object.assign({},item));existing.add(item.v);}});
  }
  ...
  function addSentences(){window.VERB_SENTENCES=window.VERB_SENTENCES||{};Object.assign(window.VERB_SENTENCES,SENTENCES);const old=window.sentenceForVerb;window.sentenceForVerb=function(v){return window.VERB_SENTENCES[v]||(typeof old==='function'?old(v):`Ich lerne ${v}.`);};}
  ...
  function classify(){
    ...
    NEW_VERBS.forEach(item=>{
      ...
      const separable=SEPARABLE.includes(item.v);
      const inseparable=INSEPARABLE.includes(item.v);
      const irregular=IRREGULAR.includes(item.v)||item.irregular===true;
    ...
    window.SP_SEPARABLE_VERBS=[...new Set([...(window.SP_SEPARABLE_VERBS||[]),...SEPARABLE])];
    window.SP_INSEPARABLE_PREFIX_VERBS=[...new Set([...(window.SP_INSEPARABLE_PREFIX_VERBS||[]),...INSEPARABLE])];
    window.SP_IRREGULAR_VERBS=[...new Set([...(window.SP_IRREGULAR_VERBS||[]),...IRREGULAR])];
    if(typeof window.STRONG_IRREGULAR_VERBS!=='undefined')IRREGULAR.forEach(v=>window.STRONG_IRREGULAR_VERBS.add(v));
    if(typeof STRONG_IRREGULAR_VERBS!=='undefined')IRREGULAR.forEach(v=>STRONG_IRREGULAR_VERBS.add(v));
    window.SP_DEFAULT_LOCKED_VERBS=[...new Set([...(window.SP_DEFAULT_LOCKED_VERBS||[]),...NEW_VERBS.map(x=>x.v)])];
  }
  addVerbs();addTranslations();addSentences();addGrammar();classify();
  window.SP_A1_EXPANSION_VERBS=[...new Set([...(window.SP_A1_EXPANSION_VERBS||[]),...NEW_VERBS.map(x=>x.v)])];
  window.SP_2026_07_VERBS=NEW_VERBS.map(x=>x.v);
})();
(function(){
'use strict';
if(window.__SP_TEACHER_RANKING_ROSTER_BACKFILL_V5)return;
window.__SP_TEACHER_RANKING_ROSTER_BACKFILL_V5=true;
...
async function studentRows(){
 ...
 try{const snap=await database.collection('students').get();const rows=snap.docs.map(doc=>({id:doc.id,__docId:doc.id,docId:doc.id,...(doc.data()||{})})).filter(usable);if(rows.length)return rows}catch(error){console.warn('Direkter Teilnehmer-Roster konnte nicht gelesen werden',error)}
 const map=window.__SP_STUDENTS_BY_ID;return map&&typeof map==='object'?Object.values(map).filter(usable):[];
}
let running=false,lastSignature='',lastRunAt=0;
async function syncRoster({force=false}={}){
 if(running)return false;const database=db();if(!database)return false;const students=await studentRows();if(!students.length)return false;
 const signature=students.map(s=>`${studentId(s)}|${courseKey(s)}|${displayName(s)}|${currentPoints(s)}`).sort().join('||');if(!force&&signature===lastSignature&&Date.now()-lastRunAt<60000)return true;
 ...
 try{
  for(let offset=0;offset<students.length;offset+=12){
   ...
   await Promise.all(chunk.map(async student=>{
    ...
    try{
     const ref=db().collection('studentRankings').doc(id);let old={};try{const snap=await ref.get();if(snap.exists)old=snap.data()||{}}catch(e){}
     ...
     await ref.set({studentId:id,authUid,displayName:name,courseKey:key,courseCode:text(student.courseCode||student.kurs||student.kursnummer),courseDocId:text(student.courseDocId),points,version:9,pointAuditVersion:auditedVersion,pointReconciliationVersion:Math.max(point(student.pointReconciliationVersion),point(old.pointReconciliationVersion)),rosterBackfilled:true,autoLoweringDisabled:true,updatedAt:nowTs()},{merge:true});written++;
    }catch(error){failed++;console.warn('Ranglisten-Roster konnte für Teilnehmer nicht gespiegelt werden',id,error)}
   }));
  }
  lastSignature=signature;lastRunAt=Date.now();try{window.dispatchEvent(new CustomEvent('SP_RANKING_ROSTER_BACKFILLED',{detail:{written,skipped,failed,total:students.length}}))}catch(e){}return failed===0;
...
function appendScript(src,marker){
 ...
 return new Promise((resolve,reject)=>{const script=document.createElement('script');script.src=src;script.async=true;script.setAttribute(`data-${marker}`,'1');script.onload=()=>resolve();script.onerror=()=>reject(new Error(`Script konnte nicht geladen werden: ${src}`));document.head.appendChild(script)});
}
async function loadPointSuite(){
 if(window.__SP_TEACHER_POINTS_SUITE_LOADING_V5)return;window.__SP_TEACHER_POINTS_SUITE_LOADING_V5=true;
 try{
  await appendScript('/teacher/points-suite.js?v=20260829-points10-final','sp-points-suite-v5');
  await appendScript('/teacher/b1-points-runtime-fix.js?v=20260829-points10-final','sp-b1-runtime-fix-v5');
 }catch(error){window.__SP_TEACHER_POINTS_SUITE_LOADING_V5=false;console.warn('Punkte-Werkzeuge konnten nicht vollständig nachgeladen werden',error)}
}
...
[300,900,1800,3500].forEach(delay=>schedule(false,delay));setTimeout(loadPointSuite,300);setInterval(()=>syncRoster({force:false}).catch(()=>{}),60000);
})();
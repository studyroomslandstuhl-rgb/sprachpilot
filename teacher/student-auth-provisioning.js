(function(root){
  ...
  async function renderPanel(){
    ...
    panel.innerHTML=`<div style="display:flex;gap:10px;align-items:center;justify-content:space-between;flex-wrap:wrap"><div><h2 style="margin:0">Schüler-Zugänge & Passwörter</h2><div style="color:#64778a">Passwörter werden nicht angezeigt oder gespeichert. Schüler legen ihr Passwort über eine sichere E-Mail selbst fest.</div></div><button id="spAuthClose" type="button">Schließen</button></div>
      ...
      <div style="overflow:auto"><table style="width:100%;border-collapse:collapse"><thead><tr><th style="text-align:left;padding:8px;border-bottom:1px solid #ddd">Schüler</th><th style="text-align:left;padding:8px;border-bottom:1px solid #ddd">E-Mail</th><th style="text-align:left;padding:8px;border-bottom:1px solid #ddd">Status</th><th style="padding:8px;border-bottom:1px solid #ddd">Aktion</th></tr></thead><tbody>${students.map(s=>{const c=core.classify(s);const disabled=c.kind==='excluded'||c.kind==='missing-email';return`<tr><td style="padding:8px;border-bottom:1px solid #eee">${esc(nameOf(s))}<div style="font-size:11px;color:#8796a4">${esc(s.__id)}</div></td><td style="padding:8px;border-bottom:1px solid #eee">${esc(c.email||'—')}</td><td style="padding:8px;border-bottom:1px solid #eee">${esc(stateLabel(c))}</td><td style="padding:8px;border-bottom:1px solid #eee;text-align:center">${disabled?'—':`<button class="sp-auth-one" data-id="${esc(s.__id)}" type="button">${c.kind==='prepared'?'Passwort-Mail erneut senden':'Zugang vorbereiten'}</button>`}</td></tr>`}).join('')}</tbody></table></div>`;
  ...
  root.StudentAuthProvisioning={api,createAccount,sendPasswordSetup,sendVerification,provisionOne,loadStudents,renderPanel,install};

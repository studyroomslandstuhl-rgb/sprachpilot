(function(){
'use strict';
if(window.__SP_TEACHER_POINT_RECOVERY_V5)return;
window.__SP_TEACHER_POINT_RECOVERY_V5=true;
...
function technicalFor(p={},module){let best=0;for(const topic of Object.values(p?.[module]||{})){if(topic&&typeof topic==='object'&&topic.technicalRecovery===true)best=Math.max(best,num(topic?.lifetime?.points))}return best}
...
function evidence(p={}){
 ...
 for(const module of MODULES){
  const base=num(calc.breakdown?.[module]),groups=groupFor(p,module),technical=technicalFor(p,module);
  total+=groups>0?Math.max(base,groups):base+technical;
 }
 total+=num(calc.breakdown?.finnischVerben);
 return total
}
 ...
 if(typeof Students!=='undefined'&&typeof Students.progressList==='function'){
  ...
  Students.progressList=async function(){
   ...
   for(const row of rows||[]){
    ...
    const nowIso=new Date().toISOString(),patch={ranking:{...(row.ranking||{}),points:target,updatedAt:nowIso},totals:{...(row.totals||{}),points:target,updatedAt:nowIso},pointsTotal:target,lifetimePoints:target,punkteGesamt:target,metadata:{...(row.metadata||{}),pointRecovery:{...(row.metadata?.pointRecovery||{}),version:6,lastRaisedAt:nowIso,raisedFrom:before,raisedTo:target,evidencePoints:fromEvidence,method:'raise-only-groups-runs-or-recovery'}},updatedAt:firebase.firestore.FieldValue.serverTimestamp()};

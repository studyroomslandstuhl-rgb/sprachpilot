(function(){
'use strict';
if(window.__SP_TEACHER_POINT_AUDIT_V1)return;window.__SP_TEACHER_POINT_AUDIT_V1=true;
...
if(window.Analytics){
 Analytics.points=function(s){return Number(calc(this.progressDoc(s)).total)||0};
}
if(window.Students&&typeof Students.progressList==='function'){
 ...
 Students.progressList=async function(){
  ...
  for(const row of rows||[]){
   ...
   try{await database.collection('progress').doc(row.id).set(patch,{merge:true});row.ranking=patch.ranking;row.totals=patch.totals;row.pointsTotal=exact;row.lifetimePoints=exact;row.punkteGesamt=exact;row.metadata=patch.metadata;changed.push({id:row.id,from:audit.stored,to:exact,inflatedBy:audit.inflatedBy})}catch(error){TeacherEnv?.note?.('Punkte konnten nicht korrigiert werden: '+row.id,error)}
  }

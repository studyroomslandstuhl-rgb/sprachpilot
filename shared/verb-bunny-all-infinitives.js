(function(){
'use strict';
if(window.__SP_VERB_BUNNY_MANIFEST_V3)return;
window.__SP_VERB_BUNNY_MANIFEST_V3=true;
...
function showError(button,verb){
 ...
 if(!error){error=document.createElement('div');error.className='bunny-audio-error';scope.appendChild(error)}
 error.textContent=`Audio für „${verb}“ ist auf diesem Gerät nicht verfügbar.`;
}
function computerSpeak(verb,slow=false){
 ...
 try{
  ...
  utterance.lang='de-DE';utterance.rate=slow?0.55:0.92;nativeSpeak(utterance);return true;
...
function play(verb,slow=false,button=null){
 ...
 const audio=new Audio(url);activeAudio=audio;audio.preload='auto';audio.playbackRate=slow?0.75:1;
 if(button)button.classList.add('bunny-audio-playing');
 let finished=false;
 const fallback=()=>{
  if(finished)return;finished=true;
  if(activeAudio===audio)activeAudio=null;clearPlaying();
  if(!computerSpeak(verb,slow))showError(button,verb);
 };
 audio.onended=()=>{finished=true;if(activeAudio===audio)activeAudio=null;clearPlaying()};
...
document.addEventListener('click',event=>{
 ...
 const url=audioUrl(verb),needsFallback=MISSING.has(normalize(verb));
 if(!url&&!needsFallback)return;
 event.preventDefault();event.stopImmediatePropagation();
 if(url)play(verb,slow,button);else computerSpeak(verb,slow);
},true);

window.SPVerbBunnyAllInfinitives={
 resolveVerb,audioUrl,urls:verb=>{const url=audioUrl(verb);return url?[url]:[]},play,computerSpeak,
 audit:window.SP_VERB_AUDIO_AUDIT||null,missing:window.SP_VERB_AUDIO_MISSING||[]
};
})();

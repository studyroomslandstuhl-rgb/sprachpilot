...
function setRun(groupId,run){localStorage.setItem(`SP_SCORE_RUN_${topicId(groupId)}`,String(Math.max(1,Math.min(3,Number(run)||1))))}
...
async function api(){if(window.SPProgress)return window.SPProgress;try{await import('/js/progress.js?v=20260831-central2')}catch(error){console.warn('Verben-Punktesynchronisierung nicht verfügbar',error)}return window.SPProgress||null}
async function sync(){
 if(E.isPreview?.())return;
 if(!await waitUntilReady())return;
 const groups=E.GROUPS||[];
 if(!groups.length)return;
 const marker=`SP_VERBEN_CLOUD_SYNC_V4_${identity()}_${hash(groups.map(group=>group.signature).join('||'))}`;
 ...
 try{
  for(const group of groups){
   ...
   const runs=Object.entries(state?.runs||{}).sort((a,b)=>Number(a[0])-Number(b[0]));
   for(const [runId,run] of runs){
    const runNo=Math.max(1,Math.min(3,Number(runId)||1));setRun(group.id,runNo);
    for(const task of E.LEARN||[]){
     ...
     await progress.recordTaskProgress({
      module:'verben',moduleTitle:'Verben',level:'A1',lesson:`Gruppe ${group.id}`,theme:`Runde ${runNo}`,
      topicId:topicId(group.id),title:`Verben · Gruppe ${group.id}`,file:task,taskKey:task,run:runNo,
    ...
    if(Number(exam.bestPercent||0)||Number(run?.awards?.examPoints||0)){
     await progress.recordExamResult({
      module:'verben',moduleTitle:'Verben',level:'A1',lesson:`Gruppe ${group.id}`,theme:`Runde ${runNo}`,
      topicId:topicId(group.id),title:`Verben · Gruppe ${group.id}`,run:runNo,percent:Number(exam.bestPercent)||0,stars:Number(exam.stars)||0
     });
    }
   }

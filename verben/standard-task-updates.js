(function(){
'use strict';
if(window.__SP_VERB_STANDARD_TASK_UPDATES_V5)return;
window.__SP_VERB_STANDARD_TASK_UPDATES_V5=true;
const E=window.VerbGroupsEngine;
if(!E)return;
function clue(v){
 const direct=window.SP_VERB_A1_MEANINGS?.[v];
 if(direct)return direct;
 const fallback=String(E.meaning?.(v)||'').trim();
 return fallback&&fallback!==v?fallback:`Was man mit „${v}“ macht`;
}

...
async function getProgress(){if(!window.SPProgress){try{await import('/js/progress.js?v=points-preserve2')}catch{}}try{return await window.SPProgress?.loadCurrentStudentProgress?.()||{}}catch{return{}}}
function groupMetadata(){
 ...
 for(const group of E?.GROUPS||[]){
  ...
  for(const[runId,run]of Object.entries(gs?.runs||{})){
   ...
   runs[runId]={tasks,awards:{tasks:{...(run?.awards?.tasks||{})},examPoints:Number(run?.awards?.examPoints)||0},exam:{bestPercent:Number(run?.exam?.bestPercent)||0,stars:Number(run?.exam?.stars)||0},completed:!!run?.completed}
  }
...
async function sync(){
 ...
 try{
  ...
  const nowIso=new Date().toISOString(),previousRecovery=progress?.metadata?.pointRecovery||{},metadata={...(progress.metadata||{}),verbenGroups:groupMetadata(),pointRecovery:{...previousRecovery,verbenApplied:carryNeeded,verbenTarget:target,verbenUpdatedAt:nowIso,method:'group-run-evidence'}};
  const next={...progress,verben,metadata},rankingPoints=allModulePoints(next),c=course(p);
  ...
  window.SP_VERBEN_FIREBASE_SYNC={ok:true,verbenPoints:actual+carryNeeded,target,rankingPoints,at:nowIso}

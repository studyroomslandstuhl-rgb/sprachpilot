...
function impliedLocalPoints(){let total=0;for(const group of E?.GROUPS||[]){const gs=E.groupState?.(group.id);if(!gs)continue;const current=Math.max(1,Math.min(3,Number(gs.currentRun)||1)),actual=Number(E.groupPoints?.(group.id))||0;let requiredPrior=0;for(let run=1;run<current;run++)requiredPrior+=(E.LEARN?.length||0)*(Number(E.taskPoints?.(run))||0)+(Number(E.examMax?.(run))||0);const currentActual=runPoints(gs.runs?.[String(current)]||{});total+=Math.max(actual,requiredPrior+currentActual)}return total}
...
async function sync(){
 ...
 try{
  ...
  const previousStored=storedPoints(progress),target=Math.max(point(E.totalPoints?.()),impliedLocalPoints());
  try{window.SPVerbRegroupRecovery?.preserveFloor?.(target)}catch{}
  const verben={...(progress.verben||{})},actual=modulePoints(verben,CARRY_ID),carryNeeded=Math.max(0,target-actual),oldCarry=verben[CARRY_ID]||{};
  if(carryNeeded>0)verben[CARRY_ID]={...oldCarry,title:'Verben · wiederhergestellte Punkte',moduleTitle:'Verben',level:'A1',technicalRecovery:true,progressPercent:Number(oldCarry.progressPercent)||0,current:{...(oldCarry.current||{}),updatedAt:new Date().toISOString()},lifetime:{...(oldCarry.lifetime||{}),points:carryNeeded}};
  else if(verben[CARRY_ID])verben[CARRY_ID]={...verben[CARRY_ID],lifetime:{...(verben[CARRY_ID].lifetime||{}),points:0}};
  const nowIso=new Date().toISOString(),previousRecovery=progress?.metadata?.pointRecovery||{},metadata={...(progress.metadata||{}),verbenGroups:groupMetadata(),pointRecovery:{...previousRecovery,verbenApplied:Math.max(point(previousRecovery.verbenApplied),carryNeeded),verbenTarget:Math.max(point(previousRecovery.verbenTarget),target),verbenUpdatedAt:nowIso,method:'group-run-evidence-raise-only'}};
  const next={...progress,verben,metadata},evidencePoints=allModulePoints(next),rankingPoints=Math.max(previousStored,evidencePoints),c=course(p);
  const patch={verben,metadata,ranking:{...(progress.ranking||{}),points:rankingPoints,updatedAt:nowIso},totals:{...(progress.totals||{}),points:rankingPoints,updatedAt:nowIso},lifetimePoints:rankingPoints,pointsTotal:rankingPoints,punkteGesamt:rankingPoints,studentId:id,userId:id,docId:id,canonicalStudentId:id,aliasIds:ids(p),studentName:displayName(p),email:p.email||progress.email||'',kurs:c||progress.kurs||'',kursnummer:c||progress.kursnummer||'',courseCode:c||progress.courseCode||'',lastActive:serverTimestamp(),updatedAt:serverTimestamp(),lastActiveAt:nowIso,lastPage:location.pathname};
  ...
  window.SP_VERBEN_FIREBASE_SYNC={ok:true,verbenPoints:actual+carryNeeded,target,evidencePoints,previousStored,rankingPoints,autoLowering:false,at:nowIso}

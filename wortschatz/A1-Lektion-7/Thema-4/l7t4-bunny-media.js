(function(){
'use strict';
if(window.__SP_L7T4_BUNNY_MEDIA_V3)return;
window.__SP_L7T4_BUNNY_MEDIA_V3=true;
...
const IMAGE_BY_WORD={
 maedchen:CDN+'maedchen.webp',
 junge:CDN+'junge.webp',
 klasse:CDN+'klasse.webp',
 schwimmbad:CDN+'schwimmbad.webp',
 eintritt:CDN+'eintritt.webp',
 grundschule:CDN+'grundschule.webp',
 unterricht:CDN+'unterricht.webp',
 leitung:CDN+'leitung.webp',
 schule:CDN+'schule.webp',
 arzt:CDN+'arzt.webp',
 aerztin:CDN+'aerztin.webp',
 ausflug:CDN+'ausflug.webp',
 schulausflug:CDN+'ausflug.webp',
 schade:CDN+'schade.webp',
 losfahren:CDN+'losfahren.webp',
 zurueckkommen:CDN+'zurueckkommen.webp',
 mitkommen:CDN+'mitkommen.webp',
 krank:CDN+'krank.webp',
 bescheid_sagen:CDN+'bescheid_sagen.webp',
 fehlen:CDN+'fehlen.webp',
 gute_besserung:CDN+'gute_besserung.webp',
 das_tut_mir_leid:CDN+'leidtun.webp',
 leidtun:CDN+'leidtun.webp'
};
...
function cleanForbidden(value){
 ...
 if(value&&typeof value==='object')Object.keys(value).forEach(key=>{value[key]=cleanForbidden(value[key])});
 return value;
}
...
function imageUrl(file){const raw=String(file||'').trim();if(/^https:\/\/sprachpilot\.b-cdn\.net\//i.test(raw))return raw;const name=basename(raw);return name?CDN+encodeURIComponent(name):''}
function install(){
...
window.L7_THEME_READY=Promise.resolve(window.L7_THEME_READY).then(theme=>{
 ...
 (cards?.items||[]).forEach(item=>{
  ...
  if(mapped)item.image=mapped;
  else if(current)item.image=imageUrl(current);
 ...
 theme.bunnyMediaRevision='l7t4-bunny-v3';window.L7_THEME=theme;return theme;
});

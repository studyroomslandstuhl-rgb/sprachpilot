(function(){
'use strict';
if(window.__SP_L7T3_REBUILD_FIXES_V4)return;window.__SP_L7T3_REBUILD_FIXES_V4=true;
...
window.L7_THEME_READY=Promise.resolve(window.L7_THEME_READY).then(theme=>{
 ...
 if(errors){
  ...
  if(errors.items?.[10])errors.items[10]={sentence:'Wir sind im Park spazieren gehen.',wrongWord:'gehen',answer:'gegangen',errorType:'Perfekt'};
 }
 const exam=(theme.tasks||[]).find(t=>t.exam);if(exam)exam.kind='';
 theme.contentRevision='l7t3-rebuild-fixes-20260822-v4';
 return theme;
});
})();

(function(){
'use strict';
const theme=Number(document.body.dataset.theme||3),page=document.body.dataset.page||'theme',root=document.getElementById('app'),version='l7t3-no-skip-v8';
...
Promise.resolve(window.L7_THEME_READY)
 ...
 .then(()=>{window.SPL7WrongQueueV7?.install?.();return load('../shared/l7-theme-score.js?v=2')})
 ...
 .then(()=>{
  ...
  return load(`../shared/l7-ui.js?v=${version}`)
   ...
   .then(()=>{
    window.SPL7WrongQueueV7?.install?.();window.L7T2MemoryUI?.install?.();window.L7T2EndingsUI?.install?.();window.L7T2AdvancedUI?.install?.();window.L7T3SpecialUI?.install?.();window.L7T3NoRepeatSpecial?.install?.();window.L7T3ErrorSelectionFix?.install?.();window.L7T3HelpStandard?.install?.();window.L7ReadingQueueFix?.install?.();window.L7T3SchonEinmalUI?.install?.();

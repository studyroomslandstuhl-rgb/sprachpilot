...
function install(){
 ...
 function next(t){const a=S.T.tasks||[];return a[a.findIndex(x=>x.id===t.id)+1]||null}
 function progress(theme,t){const st=S.load(theme,t.id,t.items.length),p=Math.round(st.done.length/Math.max(1,t.items.length)*100);return`<div class="l7-progress-row"><span>${st.done.length} von ${t.items.length} fertig</span><strong>${p}%</strong></div><div class="l7-progress"><span style="width:${p}%"></span></div>`}
 function shell(theme,t,body){return`<div class="l7-page">${S.header(theme,t.title)}<section class="l7-card">${progress(theme,t)}<div class="l7-instruction">${esc(t.description||'')}</div>${body}</section><footer>© SprachPilot</footer></div>`}
 function finish(theme,t){const n=next(t);document.getElementById('app').innerHTML=`<div class="l7-page">${S.header(theme,t.title)}<section class="l7-card l7-finish"><div>✓</div><h2>Aufgabe abgeschlossen</h2><div class="l7-actions"><a class="l7-btn secondary" href="index.html#task-${esc(t.id)}">Zur Übersicht</a>${n&&!n.exam?`<a class="l7-btn" href="task.html?task=${encodeURIComponent(n.id)}">Nächste Aufgabe</a>`:''}</div></section></div>`}
 function save(theme,t,st,sync=false){S.save(theme,t.id,st,sync)}
 function normToken(v){return String(v||'').toLowerCase().replace(/[.,!?;:“”„"'`()]/g,'').trim()}
 function help(tries,item){if(tries===1)return'<div class="l7-no">Noch nicht richtig.</div>';if(tries===2)return`<div class="l7-hint"><strong>Hinweis:</strong> ${esc(item?.errorType?`Prüfe ${item.errorType}.`:'Prüfe die Form von haben oder sein.')}</div>`;if(tries>=3)return`<div class="l7-no"><strong>Lösung:</strong> ${esc(item?.answer||'')}<br>Gib die richtige Lösung selbst ein. Die Aufgabe kommt später erneut.</div>`;return''}
 function renderSein(theme,t){
  const st=S.load(theme,t.id,t.items.length);if(st.done.length>=t.items.length)return finish(theme,t);st.answers=st.answers||{};st.answers.seinTries=st.answers.seinTries||{};st.answers.seinRepeat=st.answers.seinRepeat||{};
  const rows=t.items.map((item,i)=>{const done=st.done.includes(i),value=done?item.form:String(st.answers[`sein:${i}`]||'');return`<div class="sp-sein-row ${done?'done':''}"><strong>${esc(item.pronoun)}</strong><input data-sein="${i}" ${done?'disabled':''} value="${esc(value)}" autocomplete="off" placeholder="sein"></div>`}).join('');
  document.getElementById('app').innerHTML=shell(theme,t,`<div class="sp-sein-title">sein</div><div class="sp-sein-table">${rows}</div><button class="l7-btn sp-full" id="spSeinCheck">Prüfen</button><div id="spFeedback">${feedback}</div>`);
  document.querySelectorAll('[data-sein]').forEach(input=>input.addEventListener('input',()=>{const x=S.load(theme,t.id,t.items.length);x.answers=x.answers||{};x.answers[`sein:${input.dataset.sein}`]=input.value;save(theme,t,x,false)}));
  document.getElementById('spSeinCheck')?.addEventListener('click',()=>{const x=S.load(theme,t.id,t.items.length);x.answers=x.answers||{};x.answers.seinTries=x.answers.seinTries||{};x.answers.seinRepeat=x.answers.seinRepeat||{};let anyWrong=false,anyRight=false;t.items.forEach((item,i)=>{if(x.done.includes(i))return;const input=document.querySelector(`[data-sein="${i}"]`),v=String(input?.value||'').trim();if(!v)return;if(S.norm(v)===S.norm(item.form)){if(x.answers.seinRepeat[i]){delete x.answers.seinRepeat[i];delete x.answers[`sein:${i}`];x.answers.seinTries[i]=0;anyRight=true}else{x.done.push(i);delete x.answers[`sein:${i}`];delete x.answers.seinTries[i];anyRight=true}}else{x.answers.seinTries[i]=Number(x.answers.seinTries[i]||0)+1;x.answers.seinRepeat[i]=true;anyWrong=true}});x.current=null;save(theme,t,x,true);feedback=anyWrong?'<div class="l7-no">Korrigiere die falschen Formen. Falsch gelöste Formen müssen danach noch einmal fehlerfrei eingegeben werden.</div>':anyRight?'<div class="l7-ok">Richtig.</div>':'';renderSein(theme,t)})
 }
 function currentIndex(theme,t){const st=S.load(theme,t.id,t.items.length);if(st.done.length>=t.items.length)return null;return S.index(theme,t.id,t.items.length)}
 function renderError(theme,t){
  const index=currentIndex(theme,t);if(index==null)return finish(theme,t);const item=t.items[index],st=S.load(theme,t.id,t.items.length);st.answers=st.answers||{};const selected=Number(st.answers[`errSel:${index}`]);const hasSel=Number.isInteger(selected)&&selected>=0;const draft=String(st.answers[`errInput:${index}`]||'');const words=String(item.sentence).split(/\s+/);
  document.getElementById('app').innerHTML=shell(theme,t,`<div class="l7-question-card"><div class="sp-error-sentence">${words.map((word,i)=>`<button type="button" data-error-word="${i}" class="${hasSel&&selected===i?'selected':''}">${esc(word)}</button>`).join(' ')}</div><p class="small">1. Klicke das falsche Wort. 2. Schreibe die richtige Form.</p><div class="l7-answer-box"><label for="spErrorInput">Richtige Form</label><div><input id="spErrorInput" value="${esc(draft)}" autocomplete="off"><button class="l7-btn" id="spErrorCheck">Prüfen</button></div></div><div id="spFeedback">${feedback||help(st.tries||0,item)}</div></div>`);
  document.querySelectorAll('[data-error-word]').forEach(btn=>btn.addEventListener('click',()=>{const x=S.load(theme,t.id,t.items.length);x.answers=x.answers||{};x.answers[`errSel:${index}`]=Number(btn.dataset.errorWord);save(theme,t,x,false);renderError(theme,t)}));
  const input=document.getElementById('spErrorInput');input?.addEventListener('input',()=>{const x=S.load(theme,t.id,t.items.length);x.answers=x.answers||{};x.answers[`errInput:${index}`]=input.value;save(theme,t,x,false)});

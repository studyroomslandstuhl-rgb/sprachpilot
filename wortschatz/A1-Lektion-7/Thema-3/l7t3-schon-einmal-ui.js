(function(){
'use strict';
if(window.__SP_L7T3_SCHON_EINMAL_UI_V3)return;window.__SP_L7T3_SCHON_EINMAL_UI_V3=true;
function install(){
 if(!window.L7||!window.L7S||window.L7.__spSchonEinmalV3)return false;
 ...
 function style(){if(document.getElementById('sp-schon-style-v3'))return;const x=document.createElement('style');x.id='sp-schon-style-v3';x.textContent=`.sp-schon-cues{display:flex;gap:10px;flex-wrap:wrap;justify-content:center;margin:12px 0}.sp-schon-cues>*{padding:9px 12px;border-radius:999px;background:#f3f0ff;border:1px solid #dcd4f4}.sp-schon-mode{font-size:54px;text-align:center;margin:10px 0}.sp-schon-example{background:#faf9ff;border:1px solid #e4def8;border-radius:14px;padding:12px 14px;margin:12px 0}.sp-schon-actions{display:flex;gap:10px;flex-wrap:wrap;margin:12px 0}.sp-schon-actions .l7-btn{flex:1 1 180px}.sp-schon-input{display:none;margin-top:10px}.sp-schon-input.open{display:block}.sp-schon-input input{width:100%;padding:12px;border:2px solid #d9d4f4;border-radius:12px;font:inherit}.sp-schon-input .l7-btn{margin-top:8px;width:100%}`;document.head.appendChild(x)}
 function feedback(count,item,phase){if(count<=1)return'<div class="l7-no">Noch nicht richtig. Diese Aufgabe kommt später noch einmal.</div>';if(count===2)return`<div class="l7-hint"><strong>Hinweis:</strong> ${esc(item.hint)}</div>`;return phase==='q'?`<div class="l7-no"><strong>Lösung:</strong> ${esc(item.question)}<br>Die Frage kommt später noch einmal.</div>`:`<div class="l7-no"><strong>Lösung:</strong> ${esc(item.answer)}<br>Die Antwort kommt später noch einmal.</div>`}
 function fail(t,index,item,phase){
  const st=state(t);st.answers=st.answers||{};st.answers[phaseKey(index)]=phase;delete st.answers[draftKey(index,phase)];S.save(theme,t.id,st,false);S.attempt(theme,t.id,t.items.length,index,false);const count=S.wrong(theme,t.id,t.items.length);const fb=document.getElementById('spSchonFeedback');if(fb)fb.innerHTML=feedback(count,item,phase);document.querySelectorAll('#app button,#app input').forEach(x=>x.disabled=true)
 }
 ...
 window.L7.renderTaskPage=render;window.L7.__spSchonEinmalV3=true;return true
}

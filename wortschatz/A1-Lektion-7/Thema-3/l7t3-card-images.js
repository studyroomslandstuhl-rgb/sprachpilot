(function(){
'use strict';
if(window.__SP_L7T3_CARD_IMAGES_V1)return;
window.__SP_L7T3_CARD_IMAGES_V1=true;
...
function preload(file){
 ...
 const promise=new Promise(resolve=>{const image=new Image();image.decoding='async';image.onload=()=>resolve(true);image.onerror=()=>resolve(false);image.src=url(name)});
...
function retry(img){
 ...
 const count=Number(img.dataset.spBunnyRetry||0);if(count>=2)return fallback(img);
 img.dataset.spBunnyRetry=String(count+1);img.hidden=false;setTimeout(()=>{img.src=url(file,`${count+1}-${Date.now()}`)},180*(count+1))
}
function patchImage(img){
 ...
 img.dataset.spBunnyFile=file;img.loading='eager';img.decoding='async';
 img.onerror=function(){retry(this)};img.onload=function(){this.dataset.spBunnyRetry='0';show(this)};show(img);
 const wanted=url(file);if(!img.src||basename(img.src)!==file)img.src=wanted;else if(img.complete&&img.naturalWidth===0)retry(img)
}
function patchAll(root=document){root.querySelectorAll?.('#app img').forEach(patchImage)}
function imageHtml(file,alt='Bild'){
 const name=basename(file);if(!name)return'';preload(name);
 return `<div class="l7-image"><img src="${url(name)}" data-sp-bunny-file="${esc(name)}" loading="eager" decoding="async" alt="${esc(alt)}" onload="window.L7T3CardImages.show(this)" onerror="window.L7T3CardImages.fail(this)"><div class="l7-image-fallback" hidden><strong>${esc(alt)}</strong><span>Bild nicht verfügbar.</span></div></div>`
}
function installRenderer(){if(!window.L7S)return false;window.L7S.image=imageHtml;window.L7S.__l7t3BunnyImages=true;return true}
function collectThemeImages(theme){const files=[];(theme?.tasks||[]).forEach(task=>(task?.items||[]).forEach(item=>{if(item?.image)files.push(basename(item.image));if(item?.img)files.push(basename(item.img))}));[...new Set(files.filter(Boolean))].forEach(preload)}
window.L7_THEME_READY=Promise.resolve(window.L7_THEME_READY).then(theme=>{collectThemeImages(theme);return theme});
window.L7T3CardImages={preload,patchAll,patchImage,imageHtml,installRenderer,show,fail:retry,base:CDN};

(function(){
'use strict';
if(window.__SP_L7T2_BUNNY_AUDIO_V2)return;
window.__SP_L7T2_BUNNY_AUDIO_V2=true;
...
const PERFECT_TO_INFINITIVE=Object.freeze({
 gelernt:'lernen',gemacht:'machen',geschrieben:'schreiben',gehört:'hören',gespielt:'spielen',
 gesehen:'sehen',gelesen:'lesen',gekauft:'kaufen',gesprochen:'sprechen',gearbeitet:'arbeiten',
 getroffen:'treffen',gefrühstückt:'frühstücken',geschlafen:'schlafen',gekocht:'kochen',gegessen:'essen',
 getrunken:'trinken',gesagt:'sagen',gelebt:'leben',gekostet:'kosten',gegrillt:'grillen',gesucht:'suchen',gewohnt:'wohnen'
});
...
function infinitiveFor(value){
 const text=String(value||'').trim().toLowerCase().replace(/^(ich\s+)?(habe|hast|hat|haben|habt)\s+/,'').replace(/[.!?]+$/,'').trim();
 return PERFECT_TO_INFINITIVE[text]||'';
}
function candidates(value){
 const raw=String(value||'').trim();
 const file=basename(raw);
 const stem=file.replace(/\.mp3$/i,'');
 const plain=raw.replace(/\.mp3$/i,'').replace(/^(ich\s+)?(habe|hast|hat|haben|habt)\s+/i,'').trim();
 const infinitive=infinitiveFor(raw)||infinitiveFor(stem);
 const names=unique([
  stem,
  slug(stem,'_'),slug(stem,'-'),
  plain,slug(plain,'_'),slug(plain,'-'),
  infinitive,slug(infinitive,'_'),slug(infinitive,'-'),
  slug(normalize(raw),'_')
 ]);
 const urls=[];
 if(BUNNY.test(raw))urls.push(raw);
 if(file&&/\.mp3$/i.test(file))urls.push(BASE+encodeURIComponent(file));
 names.forEach(name=>{if(name)urls.push(BASE+encodeURIComponent(name)+'.mp3')});
 return unique(urls);
}
function errorBox(button){
 ...
 if(!box){box=document.createElement('div');box.className='bunny-audio-error';box.setAttribute('role','status');box.textContent='Die B1-Deutsch-Audiodatei konnte nicht geladen werden.';scope.appendChild(box)}
}
...
function play(value,button,done,fail){
 ...
 function next(){
  ...
  if(promise&&typeof promise.catch==='function')promise.catch(bad);
 }
 next();
}
function install(){
 ...
 window.L7S.__b1DeutschAudio=true;
 return true;
}

document.addEventListener('click',event=>{
 const button=event.target instanceof Element?event.target.closest('button[data-audio],button[data-audio-file],button[data-audio-infinitive],[data-action="card-audio"],#cardListenBtn,.card-listen-btn,.word-audio'):null;
 if(!button)return;
...
window.L7T2BunnyAudio={play,stop,candidates,base:BASE,install};

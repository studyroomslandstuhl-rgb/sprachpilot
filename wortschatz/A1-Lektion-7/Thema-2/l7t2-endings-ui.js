(function(){
'use strict';
if(window.__SP_L7T2_ENDINGS_UI_V1)return;
window.__SP_L7T2_ENDINGS_UI_V1=true;

function install(){
 if(!window.L7||!window.L7S||window.L7.__l7t2EndingsV1)return false;
 ...
 function hint(item,n){if(n<=1)return'Noch nicht richtig.';if(n===2)return'Achte auf die Endung des Partizips II.';return`Lösung: ${item.group==='t'?'-t':'-en'}.`}
 function itemHtml(theme,task,index,placed){
  ...
  return`<div class="sp-ending-row" data-ending-row="${index}"><span class="sp-ending-chip placed">${S.esc(item.infinitive)}</span><div class="sp-ending-write"><input data-ending-input="${index}" autocomplete="off" placeholder="Partizip II" value="${S.esc(draft)}"><button type="button" class="l7-btn" data-ending-check="${index}">Prüfen</button></div></div>`
 }
 ...
 function bind(theme,task){
  document.querySelectorAll('[data-ending-chip]').forEach(chip=>{
   chip.addEventListener('click',()=>{selected=Number(chip.dataset.endingChip);feedback='';render(theme,task.id)});
   chip.addEventListener('dragstart',e=>{selected=Number(chip.dataset.endingChip);e.dataTransfer?.setData('text/plain',String(selected))})
  });
 ...
 function place(theme,task,index,group){
  if(!Number.isInteger(index)||!task.items[index])return;
  const item=task.items[index],st=data(theme,task);
  if(group!==item.group){const n=Number(st.answers.tries[index]||0)+1;st.answers.tries[index]=n;S.save(theme,task.id,st,false);feedback=`<div class="${n>=2?'l7-hint':'l7-no'}">${S.esc(hint(item,n))}</div>`;selected=null;return render(theme,task.id)}
  st.answers.placements[index]=group;st.answers.tries[index]=0;S.save(theme,task.id,st,false);selected=null;feedback='';render(theme,task.id);setTimeout(()=>document.querySelector(`[data-ending-input="${index}"]`)?.focus(),30)
 }
 function check(theme,task,index,value){
  ...
  if(S.norm(value)!==S.norm(item.participle)){
   const n=Number(st.answers.tries[index]||0)+1;st.answers.tries[index]=n;S.save(theme,task.id,st,false);feedback=`<div class="${n>=2?'l7-hint':'l7-no'}">${n===1?'Noch nicht richtig.':n===2?'Achte auf die Form des Partizips II.':'Lösung: '+S.esc(item.participle)+'. Schreibe sie selbst.'}</div>`;return render(theme,task.id)
  }
  if(!st.done.includes(index))st.done.push(index);delete st.answers.drafts[index];delete st.answers.tries[index];st.current=null;S.save(theme,task.id,st,true);feedback='<div class="l7-ok">Richtig!</div>';render(theme,task.id)
 }
 const style=document.createElement('style');style.id='sp-l7t2-endings-style';style.textContent=`
 ...
 .sp-ending-chip{appearance:none;border:1px solid var(--line);border-radius:10px;background:#fff;color:var(--dark);font:inherit;font-weight:850;font-size:14px;padding:7px 10px;cursor:pointer;line-height:1.1}.sp-ending-chip.selected{outline:3px solid rgba(91,61,135,.2);border-color:var(--main)}.sp-ending-chip.placed{cursor:default}.sp-ending-chip.done{background:#e8f8ee;border-color:#52a56d}
 ...
 window.L7.__l7t2EndingsV1=true;return true
 }

(function(){
'use strict';
if(window.__SP_L7T2_MEMORY_UI_V2)return;
window.__SP_L7T2_MEMORY_UI_V2=true;
function install(){
 if(!window.L7||!window.L7S||window.L7.__l7t2MemoryV2)return false;
 ...
 function pick(theme,task,cards,id){
  ...
  const[a,b]=open;
  if(a.pair===b.pair&&a.id!==b.id){busy=true;const fresh=S.load(theme,task.id,task.items.length);if(!fresh.done.map(Number).includes(a.pair))fresh.done.push(a.pair);fresh.current=null;fresh.tries=0;fresh.hadWrong=false;S.save(theme,task.id,fresh,true);feedback='<div class="l7-ok">Richtig!</div>';setTimeout(()=>{open=[];busy=false;render(theme,task.id)},420)}
  else{busy=true;feedback='<div class="l7-no">Das passt nicht.</div>';render(theme,task.id);setTimeout(()=>{open=[];busy=false;feedback='';render(theme,task.id)},850)}
 }
 ...
 window.L7.__l7t2MemoryV2=true;return true
 }

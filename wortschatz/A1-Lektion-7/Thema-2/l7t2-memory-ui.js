(function(){
'use strict';
if(window.__SP_L7T2_MEMORY_UI_V4)return;
window.__SP_L7T2_MEMORY_UI_V4=true;
function install(){
 if(!window.L7||!window.L7S||window.L7.__l7t2MemoryV4)return false;
 ...
 function state(theme,task){
  ...
  // Alte Hilfe-/Wiederholungsmarker aus früheren Versionen bewusst entfernen.
  delete st.answers.memoryWrongPairs;
  delete st.answers.memoryHelpTries;
  return st
 }
 ...
 function pick(theme,task,cards,id){
  if(busy)return;
  const st=state(theme,task),done=new Set(st.done.map(Number)),card=cards.find(x=>x.id===id);
  if(!card||done.has(card.pair)||open.some(x=>x.id===id))return;
  open.push(card);feedback='';render(theme,task.id);if(open.length<2)return;
  const[a,b]=open;
  busy=true;
  if(a.pair===b.pair&&a.id!==b.id){
   const fresh=state(theme,task);
   if(!fresh.done.map(Number).includes(a.pair))fresh.done.push(a.pair);
   fresh.current=null;fresh.tries=0;fresh.hadWrong=false;
   S.save(theme,task.id,fresh,true);
   feedback='<div class="l7-ok">Richtig!</div>';
   render(theme,task.id);
   setTimeout(()=>{open=[];busy=false;feedback='';render(theme,task.id)},450);
   return;
  }
  // Bei einem Fehler: keine Hilfe, keine Lösung und keine spätere Wiederholung.
  feedback='<div class="l7-no">Nicht richtig.</div>';
  render(theme,task.id);
  setTimeout(()=>{open=[];busy=false;feedback='';render(theme,task.id)},700)
 }
 const style=document.createElement('style');style.id='sp-l7t2-memory-style';style.textContent=`.sp-l7t2-memory{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:10px;margin:16px 0}.sp-l7t2-memory-card{min-height:92px;border:2px solid var(--line);border-radius:14px;background:var(--soft,#f7f4fb);color:var(--dark);font:inherit;font-weight:900;cursor:pointer;padding:10px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:5px}.sp-l7t2-memory-card strong{font-size:18px}.sp-l7t2-memory-card span{font-size:11px;opacity:.68}.sp-l7t2-memory-card.selected{outline:3px solid rgba(91,61,135,.22);background:#fff}.sp-l7t2-memory-card.done{background:#e8f8ee;border-color:#52a56d;color:#245c36}@media(max-width:850px){.sp-l7t2-memory{grid-template-columns:repeat(4,minmax(0,1fr))}}@media(max-width:620px){.sp-l7t2-memory{grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}.sp-l7t2-memory-card{min-height:82px}.sp-l7t2-memory-card strong{font-size:16px}}@media(max-width:430px){.sp-l7t2-memory{grid-template-columns:repeat(2,minmax(0,1fr))}}`;
 ...
 window.L7.__l7t2MemoryV4=true;return true
 }

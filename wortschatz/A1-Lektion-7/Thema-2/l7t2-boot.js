...
Promise.resolve(window.L7_THEME_READY)
 ...
 .then(()=>{
  ...
  return load(`../shared/l7-ui.js?v=${version}`)
   ...
   .then(()=>load(`l7t2-help-standard.js?v=4`))
   .then(()=>load('../shared/l7-reading-queue-fix.js?v=2'))
   .then(()=>{window.SPL7WrongQueueV8?.install?.();window.L7T2MemoryUI?.install?.();window.L7T2EndingsUI?.install?.();window.L7T2AdvancedUI?.install?.();window.L7T2ListeningL6T4UI?.install?.();window.L7T2HelpStandard?.install?.();window.L7ReadingQueueFix?.install?.();const result=window.L7.renderTaskPage(theme,new URLSearchParams(location.search).get('task'));concise();window.L7T2CardImages?.patchAll?.(document);return result});
 })

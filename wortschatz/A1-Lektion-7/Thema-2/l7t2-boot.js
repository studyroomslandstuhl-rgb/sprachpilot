...
Promise.resolve(window.L7_THEME_READY)
 ...
 .then(()=>{
  ...
  return load(`../shared/l7-ui.js?v=${version}`).then(()=>load('../shared/l7-external-links.js?v=1')).then(()=>load(`l7t2-memory-ui.js?v=3`)).then(()=>load(`l7t2-endings-ui.js?v=3`)).then(()=>load(`l7t2-advanced-ui.js?v=1`)).then(()=>load(`l7t2-listening-l6t4-ui.js?v=3`)).then(()=>load(`l7t2-writing-variants-ui.js?v=1`)).then(()=>load(`l7t2-help-standard.js?v=3`)).then(()=>{window.L7T2MemoryUI?.install?.();window.L7T2EndingsUI?.install?.();window.L7T2AdvancedUI?.install?.();window.L7T2ListeningL6T4UI?.install?.();window.L7T2HelpStandard?.install?.();const result=window.L7.renderTaskPage(theme,new URLSearchParams(location.search).get('task'));concise();window.L7T2CardImages?.patchAll?.(document);return result});
 })

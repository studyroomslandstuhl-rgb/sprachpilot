...
Promise.resolve(window.L7_THEME_READY)
 ...
 .then(()=>{
  ...
  return load(`../shared/l7-ui.js?v=${version}`)
   ...
   .then(()=>{window.SPL7WrongQueueV7?.install?.();window.L7T2MemoryUI?.install?.();window.L7T2EndingsUI?.install?.();window.L7T2AdvancedUI?.install?.();window.L7T2ListeningL6T4UI?.install?.();window.L7T2HelpStandard?.install?.();window.L7ReadingQueueFix?.install?.();const result=window.L7.renderTaskPage(theme,new URLSearchParams(location.search).get('task'));concise();window.L7T2CardImages?.patchAll?.(document);return result});
 })

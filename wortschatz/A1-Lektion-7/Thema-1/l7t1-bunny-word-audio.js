...
const VERSION='l7t1-standardbar7';
let current=null;
let headerTimer=null;
let nativeResetButton=null;
...
function resetVisible(){return document.body?.dataset?.page!=='task'}
function captureNativeReset(){
 const button=document.querySelector('.topbar:not(.sp-standard-topbar) #resetTheme');
 if(button)nativeResetButton=button;
}
...
document.addEventListener('click',event=>{
 ...
 if(action==='reset'){if(typeof window.resetThemeProgress==='function')window.resetThemeProgress();else if(nativeResetButton)nativeResetButton.click();return;}

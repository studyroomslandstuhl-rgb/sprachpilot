...
function fallback(word,button){if(!('speechSynthesis'in window))return;try{speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(word);u.lang='de-DE';u.rate=.88;speechSynthesis.speak(u);if(button)button.title='Bunny-Datei nicht gefunden – Browseraussprache verwendet'}catch(e){}}
function play(id,word,image,button){const list=urls(id,word,image);let index=0;if(current){try{current.pause()}catch(e){}}function next(){if(index>=list.length){fallback(word||id,button);return}const audio=new Audio(list[index++]);current=audio;audio.preload='auto';audio.onerror=next;const promise=audio.play();if(promise&&promise.catch)promise.catch(next)}next()}

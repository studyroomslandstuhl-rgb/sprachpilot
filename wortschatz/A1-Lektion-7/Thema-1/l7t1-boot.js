...
function resetLegacyHelpState(){
 try{
  ...
  const pid=String(window.L7S.pid?.()||'student'),migrationKey=`SP_L7T1_HELP_FLOW_20260817_V2_${pid}`;
...
Promise.resolve(window.L7_THEME_READY)
 ...
 .then(()=>{
  if(page==='theme'){
   return load(`../shared/l7-theme-standard.js?v=${version}`).then(()=>load(`l7t1-l6-layout.js?v=${version}`)).then(()=>{window.L7ThemeStandard.render(theme);window.L7T1L6Layout?.run?.()});
  }
  ...
  return load(`../shared/l7-ui.js?v=${version}`)
   ...
   .then(()=>load(`l7t1-perfekt-ui.js?v=3`))
   ...
   .then(()=>{const result=window.L7.renderTaskPage(theme,new URLSearchParams(location.search).get('task'));window.L7T1L6Layout?.run?.();return result});
 })

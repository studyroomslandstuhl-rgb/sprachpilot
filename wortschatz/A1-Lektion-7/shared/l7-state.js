...
function audioUrl(value){const raw=String(value||'').trim();if(!raw)return'';if(/^https:\/\/sprachpilot\.b-cdn\.net\/audio\//i.test(raw))return raw;const file=raw.split(/[?#]/)[0].split('/').filter(Boolean).pop()||'';return /\.mp3$/i.test(file)?AUDIO_CDN+encodeURIComponent(file):''}
...
window.L7S={T,esc,norm,shuffle,profile,preview,load,save,index,attempt,wrong,right,markRepeat,pct,allDone,image,audioUrl,say,header,reset,mic,task,pid,run};
})();

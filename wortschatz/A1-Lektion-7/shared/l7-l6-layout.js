...
if(root){const observer=new MutationObserver(()=>run());observer.observe(root,{childList:true,subtree:true})}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',run,{once:true});else run();
window.addEventListener('load',()=>{run();setTimeout(run,120);setTimeout(run,450)});
window.L7L6Layout={run,decorateTask,decorateTheme,installPureImageBehavior};
})();

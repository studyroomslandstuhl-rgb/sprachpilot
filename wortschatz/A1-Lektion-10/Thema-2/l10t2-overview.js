...
function localPct(t){
 if(preview())return 0;
 try{
  const s=JSON.parse(localStorage.getItem(`SP_L10_${owner()}_T2_${t.id}`)||'null');
...
if(!preview()){
 try{
  ...
  if(topic){
   ...
   for(const t of tasks){
    const rec=topic.tasks?.[t.id]||topic.tasks?.[`task.html?task=${t.id}`];
    if(!rec)continue;
 ...
 }catch(e){console.warn('L10T2 overview progress',e)}
}
...
function renderTask(t,i){
 const p=Math.round(progress[t.id]||0),done=p>=100;

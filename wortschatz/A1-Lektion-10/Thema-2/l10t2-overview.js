...
const {renderSpHeader,bindSpHeader}=await import('/js/sp-header.js?v=theme-standard4');
...
function renderTask(t,i){const p=Math.round(progress[t.id]||0),done=p>=100,locked=t.exam&&!preview()&&completed<practiceTasks.length,summary=t.cardText||t.text||'';if(locked)return`<div class="l8-card l8-task-card locked" aria-disabled="true"><div class="l8-task-number">${i+1}. ${esc(t.title)}</div><div class="emoji">${esc(taskEmoji(t))}</div><p>${esc(summary)}</p><div class="l8-progress"><div style="width:0%"></div></div><div class="l8-small">gesperrt</div><div class="l8-task-start">Prüfung gesperrt</div></div>`;return`<a class="l8-card l8-task-card ${done?'done':''}" href="task.html?task=${encodeURIComponent(t.id)}"><div class="l8-task-number">${i+1}. ${esc(t.title)}</div><div class="emoji">${esc(taskEmoji(t))}</div><p>${esc(summary)}</p><div class="l8-progress"><div style="width:${p}%"></div></div><div class="l8-small">${p}%</div><div class="l8-task-start">${t.exam&&preview()?'Prüfung ansehen':done?'Fertig':'Starten'}</div></a>`}
if(root){

...
function read(){const ids=items.map(x=>x.id);let s={_run:run(),total:ids.length,done:[],order:[],wrong:{}};try{const x=JSON.parse(store().getItem(key())||'null');if(x&&Number(x._run||1)===run())Object.assign(s,x)}catch(e){}s.done=(s.done||[]).filter(x=>ids.includes(x));s.order=(s.order||[]).filter(x=>ids.includes(x));s.order=s.order.concat(shuffle(ids.filter(x=>!s.order.includes(x))));s.total=ids.length;s.wrong=s.wrong||{};return s}
let state=read();

(function(){
'use strict';
if(window.__SP_L10T2_L8_TASK_BRIDGE_V3)return;window.__SP_L10T2_L8_TASK_BRIDGE_V3=true;
...
const task=String(new URLSearchParams(location.search).get('task')||'').toLowerCase(),page=String(document.body?.dataset?.page||'');
...
function subtitle(){if(page==='overview')return'Übersicht · Gesundheit und Possessivpronomen · A1 Lektion 10 · Thema 2';if(page==='theme')return'Gesundheit und Possessivpronomen · A1 Lektion 10 · Thema 2';return`${taskTitle()} · A1 Lektion 10 · Thema 2`}
function polishHeader(){const sub=document.querySelector('.sp-header__subtitle');if(sub)sub.textContent=subtitle();document.querySelectorAll('.sp-header__nav-link').forEach(a=>{if(String(a.textContent||'').trim()==='Übersicht'&&a.tagName==='A')a.setAttribute('href','uebersicht.html')});document.querySelectorAll('#app .sp-header').forEach(h=>h.remove())}
...
const root=document.getElementById('app')||document.body;new MutationObserver(()=>requestAnimationFrame(patch)).observe(root,{childList:true,subtree:true});document.addEventListener('DOMContentLoaded',patch);[0,80,250,700,1100,1600].forEach(ms=>setTimeout(patch,ms));
})();

...
function base(){const order=shuffle(cards.map(x=>x.id));return{_run:run,flowVersion:2,order,done:[],firstCorrect:[],roundIds:[...order],round:1,placements:{},total:cards.length,notice:'',updatedAt:Date.now()}}
function load(){let s=null;try{s=JSON.parse(localStorage.getItem(KEY)||'null')}catch(e){};const legacy=!s||Number(s.flowVersion||0)!==2;if(legacy||Number(s._run||1)!==run||!Array.isArray(s.order)||s.order.length!==cards.length||cards.some(x=>!s.order.includes(x.id)))s=base();s.flowVersion=2;s.done=Array.isArray(s.done)?s.done.filter(id=>byId[id]):[];s.firstCorrect=Array.isArray(s.firstCorrect)?s.firstCorrect.filter(id=>byId[id]):[];s.roundIds=Array.isArray(s.roundIds)?s.roundIds.filter(id=>byId[id]&&!s.done.includes(id)):[];s.placements=s.placements&&typeof s.placements==='object'?s.placements:{};for(const id of Object.keys(s.placements))if(!s.roundIds.includes(id))delete s.placements[id];s.round=Math.max(1,Number(s.round||1));s.notice=String(s.notice||'');s.total=cards.length;save(s);return s}
let state=load();
function save(s=state){s._run=run;s.flowVersion=2;s.updatedAt=Date.now();localStorage.setItem(KEY,JSON.stringify(s))}

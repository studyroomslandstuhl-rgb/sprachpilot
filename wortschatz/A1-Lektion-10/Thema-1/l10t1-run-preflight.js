...
if(!window.__SP_L10T1_RUN_PATCHED){
 ...
 Storage.prototype.setItem=function(key,value){
  if(this===localStorage&&/^SP_L10_T1_/.test(String(key||''))){
   try{const data=JSON.parse(String(value||'null'));if(data&&typeof data==='object'){data._run=Math.max(1,Math.min(3,Number(localStorage.getItem('SP_SCORE_RUN_'+TOPIC)||1)||1));value=JSON.stringify(data)}}catch(e){}
  }

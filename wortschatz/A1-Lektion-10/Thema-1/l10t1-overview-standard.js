...
async function hydrateCloudProgress(){
 if(preview())return;
 try{
  const mod=await import('/js/progress.js?v=20260908-hangfix1');
  const timeout=new Promise(resolve=>setTimeout(()=>resolve(null),2500));
  const cloud=await Promise.race([mod.loadCurrentStudentProgress(),timeout]);
  ...
  for(const t of practice){
   ...
   if(recRun!==active)continue;
   const next=Math.max(progress[t.id]||0,Math.max(0,Math.min(100,Number(rec.percent||0))));
   if(next!==progress[t.id]){progress[t.id]=next;changed=true}
  }
  if(exam){
   const er=topic.exam||{};
   const erRun=Number(er.run||topic.currentRun||active)||active;
   if(erRun===active){const next=Math.max(progress[exam.id]||0,Math.max(0,Math.min(100,Number(er.percent||er.bestPercent||0))));if(next!==progress[exam.id]){progress[exam.id]=next;changed=true}}
  }
  if(changed)render();

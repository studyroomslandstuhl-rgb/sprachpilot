...
function localPct(t){try{const s=JSON.parse(localStorage.getItem(`SP_L10_T1_${t.id}`)||'null');if(!s)return 0;const active=run();if(Number(s._run||1)!==active)return 0;const total=Math.max(0,Number(s.total||34));const done=Array.isArray(s.done)?s.done.length:Math.max(0,Number(s.done||0));if(t.exam&&Number.isFinite(Number(s.percent)))return Math.max(0,Math.min(100,Number(s.percent)||0));return total?Math.min(100,Math.round(done/total*100)):0}catch(e){return 0}}
...
if(preview())for(const t of practice)progress[t.id]=100;
...
const header=renderSpHeader({subtitle:'Körper und Körperteile · A1 Lektion 10 · Thema 1',color:{main:'#F4A3A3',dark:'#A86464',soft:'#FDF1F1',line:'#F3C9C9'},navItems:[{label:'← Zurück',href:'../'}]});
root.innerHTML=`<div class="l10-theme-page">${header}<div class="l8-wrap">${teacherNote()}<section class="l8-card l8-progress-card"><div class="l8-progress-circle">${avg}%</div><div class="l8-progress-main"><h2>Dein Fortschritt</h2><p class="l8-small">${completed} / ${practice.length} Aufgaben abgeschlossen</p><div class="l8-progress"><div style="width:${avg}%"></div></div><p class="l8-small l8-theme-subtitle">Körper und Körperteile</p><div class="l8-tags"><span class="l8-tag">Körper</span><span class="l8-tag">Körperteile</span><span class="l8-tag">Artikel</span><span class="l8-tag">Plural</span></div></div><div class="l8-score-slot">${scorePanel()}</div></section><a class="l8-card l10-vocab-standard" href="uebersicht.html"><div class="emoji">📚</div><div class="l10-vocab-standard__copy"><strong>Wortschatzübersicht</strong><span>Alle 34 Wörter mit Bild, Artikel, Plural, Übersetzung und Hörfunktion.</span></div><div class="l10-vocab-standard__action">Öffnen →</div></a><section class="l8-grid">${(D.tasks||[]).map((t,i)=>{const p=Math.round(progress[t.id]||0),locked=t.exam&&!examOpen;if(locked)return`<div class="l8-card l8-task-card locked" aria-disabled="true"><div class="l8-task-number">${i+1}. ${esc(t.title)}</div><div class="emoji">${esc(taskEmoji(t))}</div><p>${esc(t.text||'')}</p><div class="l8-progress"><div style="width:0%"></div></div><div class="l8-small">gesperrt</div><div class="l8-task-start">Prüfung gesperrt</div></div>`;return`<a class="l8-card l8-task-card ${p>=100?'done':''}" href="${taskFile(t)}"><div class="l8-task-number">${i+1}. ${esc(t.title)}</div><div class="emoji">${esc(taskEmoji(t))}</div><p>${esc(t.text||'')}</p><div class="l8-progress"><div style="width:${p}%"></div></div><div class="l8-small">${p}%</div><div class="l8-task-start">${t.exam&&preview()?'Prüfung ansehen':p>=100?'Fertig':'Starten'}</div></a>`}).join('')}</section><footer>© SprachPilot</footer></div></div>`;
bindSpHeader(root);

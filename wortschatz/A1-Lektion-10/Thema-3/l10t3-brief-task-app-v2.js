...
function draw13(){const item=cur();if(!item)return finish();const opts=shuffle(item.options);shell(`<section class="l8-card l8-exercise"><div class="sp-source">${esc(item.q)}</div><h3>Wie heißt dieser Briefteil?</h3><div class="l8-options">${opts.map(o=>`<button type="button" class="l8-option" data-part="${esc(o)}">${esc(o)}</button>`).join('')}</div><div class="l8-answer-row" style="margin-top:16px"><input class="l8-input" id="article" placeholder="Artikel"><button class="l8-btn primary" id="check">Prüfen</button></div><div id="fb"></div></section>`);let chosen='';document.querySelectorAll('[data-part]').forEach(b=>b.onclick=()=>{chosen=b.dataset.part;document.querySelectorAll('[data-part]').forEach(x=>x.classList.toggle('selected',x===b))});document.getElementById('check').onclick=()=>{if(norm(chosen)===norm(item.part)&&norm(document.getElementById('article').value)===norm(item.article))right(item,draw13);else wrong(item,item.article+' '+item.part)}}

(function(){
'use strict';
if(window.__SP_L8T2_TASK4_6_20260901_V2)return;
window.__SP_L8T2_TASK4_6_20260901_V2=true;
...
const TASK6=[
 ...
 tf(TEXT3,'Lena hat heute mehr als drei Jahre Berufserfahrung nach der Ausbildung.','Falsch'),
...
function apply(theme){
 ...
 const practice=theme.tasks.filter(task=>!task?.exam),task4=practice[3],task5=practice[4],task6=practice[5];
 if(task4){task4.title='Fragen und Antworten';task4.instruction='Wähle passende Frage oder Antwort.';task4.kind='choice';task4.icon='✅';task4.emoji='✅';delete task4.intro;task4.items=TASK4.map(item=>({...item,options:[...item.options]}))}
 if(task5){task5.title='Sätze ordnen';task5.instruction='Ordne die Wörter zu einem richtigen Satz.';task5.kind='order';task5.icon='🧩';task5.emoji='🧩';delete task5.intro;task5.items=TASK5.map(item=>({...item,tokens:[...item.tokens]}))}
 if(task6){task6.id='biografien-verstehen-v2';task6.title='Biografien verstehen';task6.instruction='Lies genau und entscheide: richtig oder falsch.';task6.kind='choice';task6.icon='📖';task6.emoji='📖';delete task6.intro;task6.items=TASK6.map(item=>({...item,options:[...item.options]}))}
 return theme
}
window.L8_T2_TASK4_6_READY=Promise.resolve(window.L8_CONTENT_READY).then(themes=>{const all=window.L8_ALL_THEMES||themes||{},theme=all[2]||all['2']||(Array.isArray(all)?all.find(t=>Number(t?.number)===2):null);apply(theme);if(Number(document.body?.dataset?.theme||0)===2&&theme)window.L8_THEME=theme;return themes});window.L8_CONTENT_READY=window.L8_T2_TASK4_6_READY;window.L8T2Task4to6={apply,version:2};
})();

...
function buildMemory(items){return{id:'wortschatz-memory-bild-wort',title:'Wortschatz-Memory',instruction:'Finde die Paare: Bild und Wort.',kind:'vocab-memory',icon:'🧠',emoji:'🧠',items:items.slice(0,20).map(card=>({type:'vocab-memory-pair',...cloneCard(card)}))}}

(function(){
'use strict';
if(window.__SP_L8T2_DIRECT_BOOT_V3)return;
window.__SP_L8T2_DIRECT_BOOT_V3=true;
...
function finalize(){
 const t=theme();
 if(!t||!window.L8S||!window.L8UI){showError();return}
...
Promise.resolve(window.L8_CONTENT_READY)
 .then(()=>queueMicrotask(finalize))
 .catch(error=>{console.error('L8T2 Inhalte',error);showError()});
})();
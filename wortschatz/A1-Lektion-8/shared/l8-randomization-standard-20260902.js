(function(){
'use strict';
if(window.__SP_L8_RANDOMIZATION_STANDARD_20260902_V1)return;window.__SP_L8_RANDOMIZATION_STANDARD_20260902_V1=true;
...
function seed(){const n=themeNo(),key=`SP_L8_RANDOM_SEED_T${n}`;try{let x=sessionStorage.getItem(key);if(x)return x;x=(crypto?.randomUUID?.()||`${Date.now()}_${Math.random()}`);sessionStorage.setItem(key,x);return x}catch(e){return `${Date.now()}_${Math.random()}`}}
...
window.L8RandomizationStandard20260902={shuffle,apply,version:1};
})();

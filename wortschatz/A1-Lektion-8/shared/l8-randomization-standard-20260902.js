(function(){
'use strict';
if(window.__SP_L8_RANDOMIZATION_STANDARD_20260902_V2)return;window.__SP_L8_RANDOMIZATION_STANDARD_20260902_V2=true;
...
function seed(){const n=themeNo(),key=`SP_L8_RANDOM_SEED_T${n}`;try{let x=sessionStorage.getItem(key);if(x)return x;x=(window.crypto?.randomUUID?.()||`${Date.now()}_${Math.random()}`);sessionStorage.setItem(key,x);return x}catch(e){return `${Date.now()}_${Math.random()}`}}
function hash(text){let h=2166136261;for(const c of String(text||'')){h^=c.charCodeAt(0);h=Math.imul(h,16777619)}return h>>>0}
function shuffle(values,key){const a=[...(values||[])];let x=hash(`${seed()}|${key}`)||1;for(let i=a.length-1;i>0;i--){x=(Math.imul(x,1664525)+1013904223)>>>0;const j=x%(i+1);[a[i],a[j]]=[a[j],a[i]]}return a}
...
window.L8RandomizationStandard20260902={shuffle,apply,version:2};
})();

(function(){
'use strict';
if(window.__SP_L8T4_EXAM_FINAL_20260903_V3)return;
window.__SP_L8T4_EXAM_FINAL_20260903_V3=true;
...
function exam(){return{
 ...
 intro:'Die Aufgaben kommen in zufälliger Reihenfolge. Es gibt keine neuen Wörter und kein neues Hörverstehen.',
 items:[
...
function apply(theme){if(!theme||!Array.isArray(theme.tasks))return theme;const next=exam();const i=theme.tasks.findIndex(t=>t?.exam||String(t?.id||'')==='pruefung');if(i>=0)theme.tasks.splice(i,1,next);else theme.tasks.push(next);theme.contentRevision=String(theme.contentRevision||'')+'-exam-final-20260903-v3';if(Number(document.body?.dataset?.theme||0)===4)window.L8_THEME=theme;return theme}
const previous=window.L8_CONTENT_READY;window.L8_T4_EXAM_FINAL_READY=Promise.resolve(previous).then(themes=>{const all=window.L8_ALL_THEMES||themes||{},theme=themeOf(all);apply(theme);return themes}).catch(error=>{console.error('L8T4 Prüfung final',error);return window.L8_ALL_THEMES||{}});window.L8_CONTENT_READY=window.L8_T4_EXAM_FINAL_READY;window.L8T4ExamFinal20260903={apply,exam,version:3};
})();
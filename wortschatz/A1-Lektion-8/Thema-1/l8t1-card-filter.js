(function(){
'use strict';
if(window.__SP_L8T1_CARD_FILTER_3)return;window.__SP_L8T1_CARD_FILTER_3=true;
...
function l7Icon(task){
 ...
 if(/schreib|write|lucke|luecke|text|brief|information|markier|plural/.test(text))return'✍️';
 if(/wahl|choice|artikel|richtig|falsch|uberschrift|ueberschrift|fehler/.test(text))return'✅';
 return'✅';
}
window.L8_CONTENT_READY=Promise.resolve(window.L8_CONTENT_READY).then(themes=>{
 ...
 theme.overviewOnlyItems=overviewOnly;theme.contentRevision='l8t1-card-filter-20260825-v3';theme.iconRevision='l8t1-exact-l7-icons-v1';if(Number(document.body?.dataset?.theme)===1)window.L8_THEME=theme;return themes;
});
})();
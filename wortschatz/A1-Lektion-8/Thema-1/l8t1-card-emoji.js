(function(){
'use strict';
if(window.__SP_L8T1_CARD_EMOJI_SAFE_V2)return;
window.__SP_L8T1_CARD_EMOJI_SAFE_V2=true;
const CARD_EMOJI='🃏';
let scheduled=false;
function patchData(){
 ...
 if(cards&&cards.icon!==CARD_EMOJI)cards.icon=CARD_EMOJI;
}
...
function patchDom(){
 ...
 if(id==='karteikarten'){
  ...
  if(line){
   ...
   const instruction=raw.replace(/^[^\s]+\s*/, '').trim();
   setText(line,`${CARD_EMOJI} ${instruction}`.trim());
  }
 }
}
function schedule(){
 if(scheduled)return;
 scheduled=true;
 requestAnimationFrame(()=>{scheduled=false;patchDom()});
}

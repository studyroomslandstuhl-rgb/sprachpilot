...
function backHrefForPage(isThemeOverview=false){
  const page=(location.pathname.split('/').pop()||'index.html').toLowerCase();
  if(isThemeOverview||page==='index.html')return '../index.html';
  if(page==='statistik.html')return 'uebersicht.html';
  if(page==='uebersicht.html')return 'index.html';
  return 'index.html';
}
function header(title,isThemeOverview=false){const h=document.querySelector('.topbar');if(!h)return;let p={};try{p=JSON.parse(localStorage.getItem('SP_USER_PROFILE')||'{}')}catch(e){};const who=`${safe(p.vorname||p.firstName||'')} ${safe(p.nachname||p.lastName||'')}`.trim()||'Schüler/in';const kurs=safe(p.kurs||p.kursnummer||p.courseCode||'');const backHref=backHrefForPage(isThemeOverview);h.innerHTML=`<div class="topbar-main"><a class="brand" href="/index.html"><div class="logo"><img class="sp-logo-img" src="/assets/logo/sprachpilot-logo.png" alt="SprachPilot"></div><div><h1>SprachPilot</h1><div class="subtitle">${safe(title)} · A1 Lektion 4 · Thema 3</div></div></a><div class="account-tools"><span class="account-pill">${who}${kurs?' · '+kurs:''}</span><a class="account-link" href="/student-dashboard/index.html">Dashboard</a><a class="account-link" href="/profile/index.html">Profil</a><button class="account-link account-btn" onclick="logoutSP()">Abmelden</button></div></div><nav class="nav"><a class="btn secondary" href="${backHref}">← Zurück</a><a class="btn secondary" href="uebersicht.html">Übersicht</a><a class="btn secondary" href="statistik.html">Statistik</a><button class="btn danger-btn" onclick="resetThemeProgress()">Fortschritte löschen</button></nav>`}

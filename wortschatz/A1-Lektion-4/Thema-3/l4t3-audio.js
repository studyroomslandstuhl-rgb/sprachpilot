(function(){
  ...
  function allItems(){
    const colors=Array.isArray(window.COLORS)?window.COLORS:[];
    const adjectives=Array.isArray(window.ADJECTIVES)?window.ADJECTIVES:[];
    const reactions=Array.isArray(window.REACTIONS)?window.REACTIONS:[];
    return [...colors,...adjectives,...reactions];
  }
  function itemById(id){return allItems().find(item=>item&&item.id===id)||null}
  function itemByWord(word){
    const target=normalize(word);
    return allItems().find(item=>normalize(item&&item.word)===target)||null;
  }
  ...
  function playWord(word,slow=false,statusId='audioStatus'){
    const item=itemByWord(word);
    if(item)return playItem(item,slow,statusId);
    return fallbackSpeak(word,slow);
  }
  ...
  function buttons(item,statusId='audioStatus'){
    ...
    const sid=String(statusId||'audioStatus').replace(/'/g,"\\'");
  ...
  window.speak=function(text,slow=false){
    const item=itemByWord(text);
    if(item&&isUploaded(item))return playItem(item,slow,'audioStatus');
    return fallbackSpeak(text,slow);
  };

  if(!document.getElementById('l4t3-audio-style')){
    const style=document.createElement('style');
    style.id='l4t3-audio-style';
    style.textContent='.l4t3-audio-actions{margin:10px 0;justify-content:center}.l4t3-audio-icon{display:inline-flex;align-items:center;justify-content:center;margin-left:6px;padding:4px 7px;border:1px solid #e4bd20;border-radius:9px;background:#fff8cf;cursor:pointer;font-size:16px;line-height:1}.l4t3-audio-icon:hover{background:#ffef9a}.audio-status{min-height:24px;margin:8px 0;text-align:center;font-weight:800;color:#7a5100}.audio-status.error{color:#9b1c1c}';

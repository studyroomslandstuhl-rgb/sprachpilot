(function(){
  ...
  function audioFileId(item){return AUDIO_FILE_BY_ID[item&&item.id]||item&&item.id||''}
  function playItem(item,slow=false,statusId='audioStatus'){
    ...
    const src=AUDIO_CDN+encodeURIComponent(audioFileId(item))+'.mp3';
  ...
  window.L4T3Audio={baseColorIds:[...BASE_COLOR_IDS],uploadedIds:[...UPLOADED_IDS],audioFileById:{...AUDIO_FILE_BY_ID},itemById,itemByWord,isUploaded,playItem,playId,playWord,buttons,icon,stop};
})();

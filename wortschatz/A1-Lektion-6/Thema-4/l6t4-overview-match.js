...
function taskHref(task){
 ...
 return `${task.file}${separator}v=l6t4-match1`;
}
...
window.l6t4MatchedMenu=function(){
 ...
 grid.innerHTML='<div class="grid">'+values.map((item,index)=>{
...
window.l6t4MatchedVocabulary=function(target){
 ...
 target.innerHTML=groups.map(group=>`<section class="type-block"><div class="type-title">${group.icon||'📚'} ${group.title}</div>${group.words.map(word=>`<div class="word-row"><div class="word-placeholder" aria-hidden="true">${group.icon||'📚'}</div><div><b>${word}</b><br><span class="small">Wortschatz aus Lektion 6 · Thema 4</span></div></div>`).join('')}</section>`).join('');
};
})();
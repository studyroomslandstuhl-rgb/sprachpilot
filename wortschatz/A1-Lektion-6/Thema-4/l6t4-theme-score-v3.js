...
const isOverview=()=>{const path=location.pathname.replace(/\/+$/,'');return path.endsWith('/Thema-4')||path.endsWith('/Thema-4/index.html')||path.endsWith('/Thema-4/uebersicht.html')};
...
function practiceTasks(){
 const list=Array.isArray(window.L6T4_TASKS)?window.L6T4_TASKS:[];
 const seen=new Set();
 return list.filter(task=>task&&task.id!=='exam'&&!task.exam).filter(task=>{const key=task.key||task.file||task.id;if(!key||seen.has(key))return false;seen.add(key);return true});
}
function practiceTaskByFile(file){return practiceTasks().find(task=>task.key===file||task.file===file||task.id===file)}
...
function taskFile(id){const task=practiceTasks().find(item=>item.id===id);return task?.key||task?.file||(id==='plural'?'plural-sprechen.html':`task-${id}`)}
function taskTitleByFile(file){const task=practiceTaskByFile(file)||window.L6T4_TASKS?.find(item=>item.key===file||item.file===file||item.id===file);return task?.title||file}
function recordTask(file,state){
 ...
 const next={...old,file,title:taskTitleByFile(file),percent:Math.max(Number(old.percent)||0,percent),completed,points:completed?Math.max(Number(old.points)||0,taskPoints(run)):Number(old.points)||0,total:Math.max(Number(old.total)||0,Number(state?.total)||0),done:Math.max(Number(old.done)||0,Array.isArray(state?.done)?state.done.length:Number(state?.done)||0),updatedAt:now()};
...
function totalFor(id){const meta=practiceTasks().find(item=>item.id===id);const current=window.L6T4_DATA?.tasks?.find(item=>item.id===id);return Math.max(0,Number(meta?.total)||Number(current?.items?.length)||0)}
...
function migrate(){
 ...
 practiceTasks().forEach(task=>{const id=task.id,file=task.key||task.file||taskFile(id);let percent=rawPercent(file);if(!percent&&LEGACY_MERGES[id])percent=mergedPercent(id);if(percent>0){writeVisibleState(id,percent);const state=rawState(file);if(state)recordTask(file,state)}});
...
async function ensureApi(){if(window.SPProgress?.recordTaskProgress)return window.SPProgress;try{await import('/js/progress.js?v=l6t4-central4')}catch(e){return null}return window.SPProgress||null}
async function syncFirebase(){
 ...
 try{
  for(const key of taskKeys){const split=key.indexOf(':'),run=Math.max(1,Math.min(3,Number(key.slice(0,split))||1)),file=key.slice(split+1),item=runData(ledger,run).tasks[file];if(!item){delete ledger.pending.tasks[key];continue}localStorage.setItem(RUN_KEY,String(run));const result=await api.recordTaskProgress({module:'wortschatz',moduleTitle:'Wortschatz',level:'A1',lesson:6,theme:4,topicId:TOPIC_ID,title:'A1 Lektion 6 · Thema 4',run,file,taskKey:file,taskTitle:item.title||file,total:item.total||1,done:item.done||0,percent:item.percent||0,completed:!!item.completed});if(result)delete ledger.pending.tasks[key];else ok=false}
  for(const key of examKeys){const run=Math.max(1,Math.min(3,Number(key)||1)),data=runData(ledger,run);if(!data.examBestPercent){delete ledger.pending.exams[key];continue}localStorage.setItem(RUN_KEY,String(run));const result=await api.recordExamResult({module:'wortschatz',moduleTitle:'Wortschatz',level:'A1',lesson:6,theme:4,topicId:TOPIC_ID,title:'A1 Lektion 6 · Thema 4',run,percent:data.examBestPercent,scorePercent:data.examBestPercent,stars:data.examStars||0});if(result)delete ledger.pending.exams[key];else ok=false}
 }catch(e){ok=false}

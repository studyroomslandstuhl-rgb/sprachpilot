(function(){
'use strict';
if(window.__L6T4_CLEANUP_20260729_SAFE)return;
window.__L6T4_CLEANUP_20260729_SAFE=true;
...
function patchAllTask13(){
 try{
  [window.L6T4_META,window.L6T4_TASKS,window.L6T4_USER_META].forEach(list=>{if(Array.isArray(list))list.forEach(patchTask13)});
  const task=window.L6T4_DATA?.tasks?.find(item=>item.id==='dialog-abc');
  if(task){task.title='Dialoge';task.icon='🎧';task.description='Höre einen Dialog und beantworte alle drei Fragen.';task.external=TASK13}
 ...
 }catch(e){console.warn('L6T4 cleanup',e)}
}
patchAllTask13();
...
try{new MutationObserver(patchAllTask13).observe(document.documentElement,{childList:true,subtree:true,attributes:true,attributeFilter:['href']})}catch(e){}
})();

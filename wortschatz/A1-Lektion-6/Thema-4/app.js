...
function l6t4Load(file,total){
 try{const value=JSON.parse(l6t4Storage().getItem(l6t4TaskKey(file))||'null');if(value&&value.total===total&&Array.isArray(value.done)&&Array.isArray(value.queue))return value}catch(e){}
...
function l6t4Sync(file,state){
 ...
 try{
  ...
  window.SPProgress?.recordTaskProgress?.({module:'wortschatz',moduleTitle:'Wortschatz',level:'A1',lesson:6,theme:4,topicId:L6T4_CFG.topicId,title:L6T4_CFG.title,file:task?.file||file,taskTitle:task?.title||file,percent,done,total,completed:percent>=100})
 }catch(e){}
}
...
function l6t4Header(title,showReset=false){
 const header=document.querySelector('.topbar');if(!header)return;
 const profile=l6t4Profile(),name=`${profile?.vorname||profile?.firstName||''} ${profile?.nachname||profile?.lastName||''}`.trim()||'Schüler/in';
...
function l6t4Reset(){
 if(l6t4IsPreview()){alert('In der Lehrer-Vorschau wird kein Teilnehmerfortschritt gespeichert.');return}
 if(!confirm('Fortschritte in Lektion 6 · Thema 4 löschen? Bereits verdiente Punkte bleiben erhalten.'))return;
 const storage=l6t4Storage(),keys=[];for(let i=0;i<storage.length;i++){const key=storage.key(i);if(String(key).includes('SP_L6_T4'))keys.push(key)}keys.forEach(key=>storage.removeItem(key));
 try{window.SPProgress?.recordThemeReset?.({module:'wortschatz',moduleTitle:'Wortschatz',level:'A1',lesson:6,theme:4,topicId:L6T4_CFG.topicId,title:L6T4_CFG.title})}catch(e){}
 location.href='index.html?reset='+Date.now()
}
function l6t4RenderMenu(){

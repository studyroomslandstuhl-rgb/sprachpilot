...
function isPreview(){
 try{
  ...
  return previewFlag()&&activeRole(readProfile())==='teacher';
...
function taskTitle(file){const task=taskList().find(item=>item[0]===file);return task&&task[2]||String(file||'Aufgabe').replace(/\.html$/i,'').replace(/-/g,' ')}
...
function writeLedger(ledger,{schedule=true,dispatch=true}={}){
 ...
 if(dispatch)dispatchScore(ledger);
 if(schedule)scheduleSync();
 return ledger;
}
function statePercent(state){
 if(!state||typeof state!=='object')return 0;
 if(state.completed===true||state.finished===true)return 100;
 const total=Math.max(0,Number(state.total)||0);
 const done=Array.isArray(state.done)?state.done.length:Math.max(0,Number(state.done)||0);
 return total?clamp(done/total*100):clamp(state.percent??state.progress);
}
...
function applyTaskState(ledger,run,file,state){
 ...
 const next={...old,file,title:taskTitle(file),percent:Math.max(clamp(old.percent),snap.percent),completed,points:completed?Math.max(Number(old.points)||0,taskPoints(run)):Number(old.points)||0,total:Math.max(Number(old.total)||0,snap.total),done:Math.max(Number(old.done)||0,snap.done),updatedAt:now()};
...
function recordTask(file,state){
 if(isPreview()||file===EXAM_FILE)return readLedger();
 const ledger=readLedger();
 if(!applyTaskState(ledger,ledger.currentRun,file,state))return ledger;
...
function taskStateCandidates(file){
 ...
 try{
  for(let i=0;i<localStorage.length;i++){
   ...
   if((key.startsWith('SP_L6_T3_V1_')||key.startsWith('SP_TASK_STATE_')||key.startsWith('SP_TASK_PROGRESS_'))&&key.includes(file)){
    const value=readState(key);if(value)out.push(value);
   }
  }
  if(typeof window.taskKey==='function'){
   const direct=readState(window.taskKey(file));if(direct)out.push(direct);
  }
  [file,cleanId(file)].forEach(name=>{
   ['SP_TASK_STATE_','SP_TASK_PROGRESS_'].forEach(prefix=>{const value=readState(prefix+name);if(value)out.push(value)});
  });
 }catch(e){}
 return out;
}
function reconcile(){
 ...
 const ledger=readLedger(),run=ledger.currentRun;
 let changed=false;
 taskList().forEach(task=>{
  const file=task[0],candidates=taskStateCandidates(file);
  if(!candidates.length)return;
  const best=candidates.reduce((winner,state)=>statePercent(state)>statePercent(winner)?state:winner,candidates[0]);
  if(statePercent(best)>0&&applyTaskState(ledger,run,file,best))changed=true;
 });
 localStorage.setItem(RUN_KEY,String(ledger.currentRun));
 if(changed)return writeLedger(ledger);
 dispatchScore(ledger);
 return ledger;
}
...
function clearVisibleProgress(){
 ...
 const protectedKeys=new Set([LEDGER_KEY,RUN_KEY]);
 const files=new Set(taskList().map(task=>task[0]).concat([EXAM_FILE]));
 const remove=[];
 for(let i=0;i<localStorage.length;i++){
  const key=localStorage.key(i)||'';
  if(protectedKeys.has(key))continue;
  if(key.startsWith('SP_L6_T3_V1_')||[...files].some(file=>key==='SP_TASK_STATE_'+file||key==='SP_TASK_PROGRESS_'+file))remove.push(key);
 }
 remove.forEach(key=>localStorage.removeItem(key));
 return{localRemoved:remove,sessionRemoved:[],total:remove.length};
}
function resetPractice(){
 ...
 const ledger=readLedger();localStorage.setItem(RUN_KEY,String(ledger.currentRun));
 location.replace('index.html?resetDone='+Date.now()+'&v=l6t3-score1');
 return true;
}
function startNextRun(){
 const ledger=readLedger(),current=ledger.currentRun,data=runData(ledger,current);
 ...
 ledger.currentRun=next;runData(ledger,next);
 clearVisibleProgress();
 writeLedger(ledger);
 location.replace('index.html?v=l6t3-score1&run='+next);
 return true;
}
...
function summaryHtml(){
 ...
 return '<div class="score-ledger-card"><div><b>Punkterunde '+s.currentRun+' von 3</b><div class="small">Aufgaben: '+s.runTaskPoints+' Punkte · Prüfung: '+s.runExamPoints+' Punkte</div></div><div class="score-ledger-total">Gesamt: '+s.lifetimePoints+' Punkte</div>'+(s.pending?'<div class="small">Synchronisierung vorgemerkt</div>':'')+(s.canRepeat?'<div class="actions"><button class="btn" type="button" onclick="L6T3ThemeScore.startNextRun()">Thema wiederholen – Runde '+next+' starten</button></div>':'')+'</div>';
}
async function ensureProgressApi(){
 ...
 try{await import('/js/progress.js?v=l6t3-score1')}catch(e){return null}
 return window.SPProgress||null;
}
async function syncFirebase(){
 if(isPreview()||syncing)return false;
 const ledger=readLedger(),taskKeys=Object.keys(ledger.pending.tasks||{}),examKeys=Object.keys(ledger.pending.exams||{});
 if(!taskKeys.length&&!examKeys.length)return true;
 const api=await ensureProgressApi();if(!api)return false;
 syncing=true;let ok=true;
 try{
  for(const key of taskKeys){
   const split=key.indexOf(':'),run=Math.max(1,Math.min(3,Number(key.slice(0,split))||1)),file=key.slice(split+1),data=runData(ledger,run),task=data.tasks[file];
   if(!task){delete ledger.pending.tasks[key];continue}
   localStorage.setItem(RUN_KEY,String(run));
   const result=await api.recordTaskProgress({module:'wortschatz',moduleTitle:'Wortschatz',level:'A1',lesson:6,theme:3,topicId:TOPIC_ID,title:'A1 Lektion 6 · Thema 3',file,taskKey:file,taskTitle:task.title||taskTitle(file),total:task.total||1,done:task.done||0,percent:task.percent||0,completed:!!task.completed});
   if(result)delete ledger.pending.tasks[key];else ok=false;
  }
  for(const key of examKeys){
   const run=Math.max(1,Math.min(3,Number(key)||1)),data=runData(ledger,run);
   if(!data.examBestPercent){delete ledger.pending.exams[key];continue}
   localStorage.setItem(RUN_KEY,String(run));
   const result=await api.recordExamResult({module:'wortschatz',moduleTitle:'Wortschatz',level:'A1',lesson:6,theme:3,topicId:TOPIC_ID,title:'A1 Lektion 6 · Thema 3',percent:data.examBestPercent,stars:data.examStars||0});
  ...
  syncing=false;dispatchScore(ledger);
 }
 return ok;
}

...
function apply(){
 ...
 const localTheme=window[`L9T${n}`];
 if(taskId&&localTheme?.tasks){const task=localTheme.tasks.find(x=>String(x.id)===String(taskId));if(task?.title)title=task.title}

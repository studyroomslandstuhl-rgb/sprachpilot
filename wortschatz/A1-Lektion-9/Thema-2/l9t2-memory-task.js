...
function currentGroup(s){return groups(s.memoryOrder).find(g=>g.some(id=>!s.done.includes('mem-'+id)))||null}
...
function draw(){const s=read(),group=currentGroup(s);if(!group)return finish(s);const pendingIds=group.filter(id=>!s.done.includes('mem-'+id));const roundWords=pendingIds.map(id=>words.find(w=>w.id===id)).filter(Boolean);const cards=shuffle(roundWords.flatMap(w=>[{k:w.id,t:'img',w},{k:w.id,t:'word',w}]));let open=[],matched=new Set(),locked=false;const roundNo=groups(s.memoryOrder).findIndex(g=>g===group)+1;root.innerHTML=`<div class="l8-wrap">${preview()?'<div class="sp-teacher-preview-note">Lehrer-Vorschau: Teilnehmerfortschritt wird nicht gespeichert.</div>':''}${head(s)}<section class="l8-card l8-exercise"><div class="l9t2-review-note">Runde ${roundNo} von 3 · Finde immer ein Bild und das passende Wort.</div><div class="l9t2-memory">${cards.map((c,i)=>`<button class="l9t2-memory-card" data-i="${i}" type="button"><span>?</span></button>`).join('')}</div><div id="feedback"></div></section><footer>© SprachPilot</footer></div>`;
 const btns=[...document.querySelectorAll('[data-i]')];
 function reveal(i,on){const b=btns[i],c=cards[i];if(!b)return;if(on){b.classList.add('open');b.innerHTML=c.t==='img'?`<img src="${esc(c.w.image)}" alt="">`:`<span class="l9t2-memory-word">${esc(label(c.w))}</span>`}else{b.classList.remove('open');b.innerHTML='<span>?</span>'}}

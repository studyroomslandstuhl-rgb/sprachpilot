...
function draw(){const s=read(),info=currentGroupInfo(s);if(!info)return finish(s);const group=info.group,pendingIds=group.filter(id=>!s.done.includes('mem-'+id));const roundWords=pendingIds.map(id=>words.find(w=>w.id===id)).filter(Boolean);const cards=shuffle(roundWords.flatMap(w=>[{k:w.id,t:'img',w},{k:w.id,t:'word',w}]));let open=[],matched=new Set(),locked=false;const roundNo=info.index+1;root.innerHTML=`<div class="l8-wrap">${preview()?'<div class="sp-teacher-preview-note">Lehrer-Vorschau: Teilnehmerfortschritt wird nicht gespeichert.</div>':''}${head(s)}<section class="l8-card l8-exercise"><div class="l9t2-review-note">Runde ${roundNo} von 3 · Finde immer ein Bild und das passende Wort.</div><div class="l9t2-memory">${cards.map((c,i)=>`<button class="l9t2-memory-card" data-i="${i}" type="button"><span>?</span></button>`).join('')}</div><div id="feedback"></div></section><footer>© SprachPilot</footer></div>`;

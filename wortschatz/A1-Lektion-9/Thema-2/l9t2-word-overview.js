...
const items=(D.cards||[]).map(item=>{const base=canonical.get(item.id)||{};return {...base,...item,perfect:item.perfect||base.perfect||'',plural:item.plural||base.plural||'',image:item.image||base.image||`https://sprachpilot.b-cdn.net/${item.id}.webp`,audio:item.audio||base.audio||`https://sprachpilot.b-cdn.net/audio/${item.id}.mp3`}});

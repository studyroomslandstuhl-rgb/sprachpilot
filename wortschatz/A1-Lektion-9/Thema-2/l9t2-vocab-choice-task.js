...
const prefix=taskId==='bild-wort'?'pic-':'aud-',valid=words.map(x=>prefix+x.id);
function read(){let s={done:[],wrong:{},review:{},answers:{},order:[]};try{s={...s,...JSON.parse(store.getItem(key)||'{}')}}catch(e){}s.done=[...new Set((s.done||[]).filter(x=>valid.includes(x)))];s.wrong=s.wrong||{};s.review=s.review||{};s.answers=s.answers||{};s.order=(s.order||[]).filter(x=>valid.includes(x));const missing=valid.filter(x=>!s.order.includes(x));if(missing.length||s.order.length!==valid.length)s.order=[...s.order,...shuffle(missing)];write(s,false);return s}

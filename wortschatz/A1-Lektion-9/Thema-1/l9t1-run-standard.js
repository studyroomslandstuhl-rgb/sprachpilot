...
function clearAll(){const prefix=`SP_L9_${pid()}_T1_`,keys=[];for(let i=0;i<localStorage.length;i++){const k=String(localStorage.key(i)||'');if(k.startsWith(prefix))keys.push(k)}keys.forEach(k=>localStorage.removeItem(k))}
function clearExam(){localStorage.removeItem(`SP_L9_${pid()}_T1_pruefung`)}

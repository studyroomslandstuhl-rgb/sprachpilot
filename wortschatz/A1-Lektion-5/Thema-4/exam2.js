...
function examPct(file,total){
 if(file==='zuordnen.html'){
  try{window.SP_L5T4_TASK10_MIGRATE?.()}catch(e){}
 }
 return pctFor(file,total);
}
const ok=need.every(x=>examPct(x[0],x[1])>=100);
const q=[
...
let i=0,score=0;
if(!ok){area.innerHTML='<div class="question">Prüfung noch gesperrt</div><div class="hint">Bearbeiten Sie zuerst alle Aufgaben.</div><div class="exam-lock">'+need.map(x=>'<a href="'+x[0]+'">'+x[0].replace('.html','')+': '+examPct(x[0],x[1])+'%</a>').join('')+'</div><div class="actions"><a class="btn secondary" href="index.html">Zum Menü</a></div>'}else{show()}

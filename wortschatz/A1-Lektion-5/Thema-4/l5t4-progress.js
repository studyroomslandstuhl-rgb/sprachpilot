...
function knownTotal(file,fallback){const fixed=Number(TASK_TOTALS[canonical(file)])||0;return Math.max(0,Number(fallback)||fixed||0)}
...
window.spL5T4SyncNow=async()=>{const api=await loadApi();let master=readMaster();master=await applyPendingResets(api,master);master=await pullCloud(api);syncAll(master);return flushCloud()};

writeMaster(readMaster(),false);
loadApi().then(async api=>{let master=readMaster();master=await applyPendingResets(api,master);master=await pullCloud(api);syncAll(master);flushCloud()});

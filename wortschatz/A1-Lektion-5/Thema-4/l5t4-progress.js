(function(){
'use strict';
if(window.__SP_L5T4_PROGRESS_V2)return;
window.__SP_L5T4_PROGRESS_V2=true;
...
let cloudPromise=null,flushTimer=null;
...
function runFromStorage(){return Math.max(1,Math.min(3,Math.round(Number(localStorage.getItem(SCORE_RUN_KEY)||1)||1)))}
function taskPoints(run){return run===1?5:run===2?10:run===3?15:0}
function examMax(run){return run===1?100:run===2?200:run===3?300:0}
function knownTotal(file,fallback){const fixed=TASK_TOTALS[canonical(file)];return Math.max(0,Number(fallback??fixed)||0)}
...
function emptyMaster(){return{version:2,run:runFromStorage(),tasks:{},exam:{byRun:{}},awards:{taskByRun:{},examByRun:{}},points:0,updatedAt:new Date().toISOString()}}
...
function ensure(master){
 master.version=2;master.tasks=master.tasks||{};master.exam=master.exam||{byRun:{}};master.exam.byRun=master.exam.byRun||{};
 ...
 master.run=Math.max(1,Math.min(3,Math.max(Number(master.run)||1,runFromStorage())));
...
function awardCompleted(master){
 for(const[file,st]of Object.entries(master.tasks)){if(!TASK_TOTALS.hasOwnProperty(file)||percentOf(st,st.total)<100)continue;master.awards.taskByRun[file]=master.awards.taskByRun[file]||{};const r=String(master.run);if(!master.awards.taskByRun[file][r])master.awards.taskByRun[file][r]=taskPoints(master.run)}
}
...
function loadApi(){if(window.SPProgress)return Promise.resolve(window.SPProgress);if(cloudPromise)return cloudPromise;cloudPromise=import('/js/progress.js?v=l5t4-central2').then(()=>window.SPProgress||null).catch(e=>{console.warn('L5T4: Firebase-Sync nicht verfügbar',e);cloudPromise=null;return null});return cloudPromise}
...
function queueCloud(method,data,key,extra=''){
 key=key||method+':'+(data?.file||'');const sig=signature(method,data,extra),storeKey=syncStorageKey(key);if(localStorage.getItem(storeKey)===sig)return;
 pending.set(key,{method,data,key,sig,storeKey});clearTimeout(flushTimer);flushTimer=setTimeout(flushCloud,120);
}
async function flushCloud(){
 if(!pending.size)return;const api=await loadApi();if(!api){setTimeout(flushCloud,1500);return}
 const items=[...pending.values()];pending.clear();
 for(const item of items){try{const fn=api[item.method];if(typeof fn!=='function')continue;await fn(item.data);localStorage.setItem(item.storeKey,item.sig)}catch(e){console.warn('L5T4: Sync fehlgeschlagen',item.method,e);pending.set(item.key,item)}}
 if(pending.size)setTimeout(flushCloud,1200);
}
async function alignRun(api,master){
 if(!api||typeof api.currentRun!=='function'||typeof api.recordThemeReset!=='function')return;
 let cloudRun=Math.max(1,Math.min(3,Number(api.currentRun(TOPIC_ID))||1));
 while(cloudRun<master.run){await api.recordThemeReset({module:'wortschatz',moduleTitle:'Wortschatz',level:'A1',lesson:'5',theme:'4',topicId:TOPIC_ID,title:'A1 Lektion 5 · Thema 4'});cloudRun=Math.max(1,Math.min(3,Number(api.currentRun(TOPIC_ID))||cloudRun+1))}
}
...
async function pullCloud(api){
 ...
 try{const progress=await api.loadCurrentStudentProgress(),topic=progress?.wortschatz?.[TOPIC_ID];if(!topic)return master;const cloudRun=Math.max(1,Math.min(3,(Number(topic?.lifetime?.resets)||0)+1));master.run=Math.max(master.run,cloudRun);mergeAwards(master,topic);const cloudTasks=topic.tasks||{};
...
function syncAll(master=readMaster()){
 for(const file of Object.keys(TASK_TOTALS)){const st=master.tasks[file];if(st&&st.total&&percentOf(st,st.total)>0)queueCloud('recordTaskProgress',taskPayload(file,st),'task:'+file)}
 const ex=master.exam?.byRun?.[String(master.run)];if(ex&&ex.attempts){const data={module:'wortschatz',moduleTitle:'Wortschatz',level:'A1',lesson:'5',theme:'4',topicId:TOPIC_ID,title:'A1 Lektion 5 · Thema 4',file:'pruefung.html',taskTitle:'Prüfung',score:ex.score||0,maxScore:ex.maxScore||12,percent:ex.percent||0,scorePercent:ex.percent||0,stars:ex.stars||0};queueCloud('recordExamResult',data,'exam:'+master.run,'attempt:'+ex.attempts)}
}
function resetTheme(){
 if(!confirm('Fortschritte in diesem Thema löschen?'))return;const master=readMaster();master.run=Math.min(3,master.run+1);master.tasks={};master.exam.byRun[String(master.run)]={attempts:0,percent:0,score:0,maxScore:12,stars:0};writeMaster(master);for(const key of Object.keys(localStorage))if(key.startsWith('SP_L5_T4_SYNC_'))localStorage.removeItem(key);
 loadApi().then(async api=>{await alignRun(api,master);location.href='index.html'}).catch(()=>{location.href='index.html'});setTimeout(()=>{if(!/\/index\.html$/.test(location.pathname))location.href='index.html'},1800)
}

window.taskKey=file=>'SP_L5_T4_CANONICAL_'+canonical(file);
window.loadTask=(file,total)=>loadTaskState(file,total);
window.saveTask=(file,st)=>saveTask(file,st,true);
window.markTaskDone=(file,total)=>saveTask(file,{total,done:[...Array(total).keys()],queue:[],current:null,tries:0,hadWrong:false},file!=='pruefung.html');
window.pctFor=function(file,total){file=canonical(file);if(file==='pruefung.html')return currentExamPercent();const st=loadTaskState(file,total);return percentOf(st,total||st.total)};
window.spProgressHtml=function(file,total){const st=loadTaskState(file,total),d=Math.min(st.done.length,total),p=window.pctFor(file,total);return`<div class="small">${d} richtig · ${Math.max(0,total-d)} übrig · ${p}%</div><div class="progress"><div class="bar" style="width:${p}%"></div></div>`};
window.resetThemeProgress=resetTheme;
window.spL5T4SaveExamResult=saveExam;
window.spL5T4ReadProgress=readMaster;
window.spL5T4ThemePoints=()=>readMaster().points||0;
window.spL5T4SyncNow=async()=>{const api=await loadApi(),master=await pullCloud(api);await alignRun(api,master);syncAll(master);return flushCloud()};

writeMaster(readMaster(),false);
loadApi().then(async api=>{let master=await pullCloud(api);await alignRun(api,master);master=readMaster();syncAll(master);flushCloud()});
window.addEventListener('online',()=>window.spL5T4SyncNow());
window.addEventListener('focus',()=>{syncAll();flushCloud()});
document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible'){syncAll();flushCloud()}});
})();

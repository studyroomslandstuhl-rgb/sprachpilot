(function(){
'use strict';
if(window.__SP_L5T4_TASK10_PROGRESS_SCOPE_V6)return;window.__SP_L5T4_TASK10_PROGRESS_SCOPE_V6=true;
...
function isTask10(file){return /^zuordnen(?:-v[56])?\.html$/i.test(String(file||''))}
function scopedTaskKey(file){if(isTask10(file))return `${cfgKey}_U_${pid()}_zuordnen_v6`;return rawTaskKey?rawTaskKey(file):`${cfgKey}_${file}`}
if(rawTaskKey){window.taskKey=scopedTaskKey;try{taskKey=scopedTaskKey}catch(e){}}
window.SP_L5T4_TASK10_PID=pid;
window.SP_L5T4_TASK10_KEY=function(file='zuordnen-v6.html'){return scopedTaskKey(file)};
})();
(function(){
  ...
  function loadProgress(){
    ...
    s.type='module';
    s.src='/js/progress.js?v=l5t1-light2';
  ...
  function localState(file){try{return JSON.parse(localStorage.getItem(stateKey(file))||'null')}catch(e){return null}}
  function isComplete(st){return !!(st&&st.total&&Array.isArray(st.done)&&st.done.length>=st.total)}
  function sync(file,st){
    try{
      if(!isComplete(st))return;
      queue(file==='pruefung.html'?'recordExamResult':'recordTaskProgress',payload(file,st.total,st.done.length));
    }catch(e){}
  }
  const oldSave=window.saveTask;
  if(typeof oldSave==='function'){
    window.saveTask=function(file,st){
      const before=localState(file);
      oldSave(file,st);
      try{if(!isComplete(before)&&isComplete(st))sync(file,st);}catch(e){}
    };
  }
  const oldComplete=window.complete;
  if(typeof oldComplete==='function'){
    window.complete=function(area,file,next){return oldComplete(area,file,next);};
  }
  window.markTaskDone=function(file,total){const before=localState(file);let st=window.loadTask(file,total);st.done=[...Array(total).keys()];st.queue=[];st.current=null;st.tries=0;st.hadWrong=false;window.saveTask(file,st);if(!isComplete(before))sync(file,st)};

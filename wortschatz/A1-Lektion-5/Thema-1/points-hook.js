(function(){
  ...
  const TASK_TITLES={
    ...
    'hoeren.html':'Hören','sehen-schreiben.html':'Sehen → Schreiben','sprechen.html':'Sprechen','formell-informell.html':'formell ↔ informell','frage-antwort.html':'Frage / Antwort','schon-erst.html':'schon / erst'
  };
  ...
  function run(){return Math.max(1,Math.round(Number(localStorage.getItem('SP_SCORE_RUN_'+topicId())||1)||1))}
  function loadProgress(){if(window.__spL5ProgressLoading)return;window.__spL5ProgressLoading=true;const s=document.createElement('script');s.type='module';s.src='/js/progress.js?v=l5-points-1';document.head.appendChild(s)}
  ...
  function payload(file,percent,total,done){return {module:'wortschatz',moduleTitle:'Wortschatz',level:'A1',lesson:'5',theme,themeId:topicId(),title:topicTitle,file,taskKey:file,taskTitle:TASK_TITLES[file]||file.replace('.html',''),percent:Math.max(0,Math.min(100,Math.round(Number(percent)||0))),completed:Number(percent)>=100,total:Number(total||0),done:Number(done||0)}}
  ...
  function marker(kind,file,pct){return `SP_L5_POINTS_SYNC_${topicId()}_${kind}_${file}_${run()}_${pct}`}
  function syncTask(file,st){const pct=percent(st),total=totalCount(st),done=doneCount(st);if(!total||pct<100)return;const m=marker('task',file,100);if(localStorage.getItem(m)==='1')return;localStorage.setItem(m,'1');queue('recordTaskProgress',payload(file,100,total,done))}
  function syncExam(file,st){const total=totalCount(st),done=doneCount(st);if(!total||done<=0)return;const pct=percent(st);const m=marker('exam',file,pct);if(localStorage.getItem(m)==='1')return;localStorage.setItem(m,'1');const p=payload(file,pct,total,done);p.scorePercent=pct;p.score=pct;p.stars=pct>=100?3:pct>=70?2:pct>=50?1:0;queue('recordExamResult',p)}
  ...
  window.syncExam=function(result){const pct=Math.max(0,Math.min(100,Math.round(Number(result&&result.percent)||0)));const total=Number(result&&result.maxScore||result&&result.total||100);const done=Math.round(total*pct/100);const p=payload('pruefung.html',pct,total,done);p.score=Number(result&&result.score||done);p.maxScore=total;p.scorePercent=pct;p.stars=Number(result&&result.stars||(pct>=100?3:pct>=70?2:pct>=50?1:0));const m=marker('exam','pruefung.html',pct);if(localStorage.getItem(m)!=='1'){localStorage.setItem(m,'1');queue('recordExamResult',p)}};
  scan();setTimeout(scan,400);setTimeout(scan,1500);setTimeout(scan,4000);setInterval(scan,5000);
})();
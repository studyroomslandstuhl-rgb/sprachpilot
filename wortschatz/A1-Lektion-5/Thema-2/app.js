...
function voice(kind){const vs=speechSynthesis.getVoices().filter(v=>(v.lang||'').toLowerCase().startsWith('de'));if(!vs.length)return null;const male=/markus|michael|hans|stefan|daniel|thomas/i,female=/anna|katja|helena|petra|marlene|google/i;return vs.find(v=>(kind==='m'?male:female).test(v.name))||vs[kind==='m'?1:0]||vs[0]}
function utter(txt,kind='f'){const u=new SpeechSynthesisUtterance(txt);u.lang='de-DE';u.rate=.92;u.pitch=kind==='m'?.85:1.08;const v=voice(kind);if(v)u.voice=v;return u}
function sayGerman(txt,kind='f'){speechSynthesis.cancel();speechSynthesis.speak(utter(txt,kind))}
function speakDialog(lines,i=0){if(i===0)speechSynthesis.cancel();if(i>=lines.length)return;const line=typeof lines[i]==='string'?{v:i%2?'m':'f',t:lines[i]}:lines[i];const u=utter(line.t,line.v||'f');u.onend=()=>setTimeout(()=>speakDialog(lines,i+1),180);speechSynthesis.speak(u)}

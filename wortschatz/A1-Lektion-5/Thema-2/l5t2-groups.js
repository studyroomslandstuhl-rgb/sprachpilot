(function(){
  ...
  window.renderStats=function(el){
    ...
    el.innerHTML=grouped().map(([label,items])=>`<section class="type-block"><div class="type-title">${label}</div>${items.map(w=>{const i=WORDS.indexOf(w);return`<div class="word-row">${imgHtml(w)}<div><b>${full(w)}</b>${pluralLine(w)}<div class="native-trans"><b>Muttersprache:</b> ${tr(w)}</div><span class="tag">${label}</span><span class="small ${st.done.includes(i)?'ok':'todo'}">${st.done.includes(i)?'gelernt':'noch offen'}</span></div></div>`}).join('')}</section>`).join('');
  };
})();
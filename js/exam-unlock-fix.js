...
function isTaskComplete(t){return !!(t&&(t.completed===true||clamp(t.percent)>=100||(Number(t.total||0)>0&&Number(t.done||0)>=Number(t.total||1))))}
...
function topicAllowsExam(topic){if(!topic)return false;const allKnown=Object.values(topic.tasks||{}).filter(t=>!String(t.file||t.key||"").includes("pruefung"));if(allKnown.length>0)return allKnown.every(isTaskComplete);const taskPercent=clamp(topic.taskPercent||topic.current&&topic.current.taskPercent||topic.current&&topic.current.percent||topic.progressPercent||0);const completed=Number(topic.completedTasks||topic.current&&topic.current.completedTasks||0);const total=Number(topic.totalTasks||topic.current&&topic.current.totalTasks||0);return taskPercent>=100||(total>0&&completed>=total)}
...
function patchPercentReaders(files={}){Object.assign(window.__SP_EXAM_UNLOCK_FILES,files);const done=file=>!!window.__SP_EXAM_UNLOCK_FILES[String(file||"")];
...
function rerenderOrReload(){try{if(typeof window.renderPage==="function")window.renderPage()}catch(e){}try{if(typeof window.renderMenu==="function")window.renderMenu()}catch(e){}try{if(typeof window.render==="function")window.render()}catch(e){}try{if(typeof window.show==="function")window.show()}catch(e){}if(IS_EXAM&&sessionStorage.getItem(STORE_PREFIX+PATH)!=="1"){sessionStorage.setItem(STORE_PREFIX+PATH,"1");setTimeout(()=>location.reload(),150)}}

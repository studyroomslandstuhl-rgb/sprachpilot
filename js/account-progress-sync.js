...
async function prepareSecureIdentity(timeout=5000){
  const until=Date.now()+Math.max(1000,Number(timeout)||5000);let lastError=null;
  while(Date.now()<until){
    ...
    if(secureStudentOwner())return{ok:true,user};
    if(user&&!user.isAnonymous&&user.emailVerified===true){
      try{await normalizeStudentIdentity(null,{silent:true});lastError=null}
      catch(error){
        lastError=error;
...
function refreshAfterProgressPreparation(result,isolation){
  ...
  const key='SP_ACCOUNT_PROGRESS_RENDERED_V12_'+studentId+'_'+page;
...
async function startInner(options={}){
  ...
  if(result?.reason==='STUDENT_AUTH_PENDING'){scheduleRetry(1200);return inactiveResult(result.reason)}
  let restoredL78=0;

...
if(!window.__SP_RANKING_MIRROR_V1){
  window.__SP_RANKING_MIRROR_V1=true;
  ...
  async function mirror(total){const p=profile(),sid=id(p),pts=point(total);if(!sid||pts<=0)return false;const c=course(p);try{await setDoc(doc(db,'students',String(sid)),{studentId:p.studentId||p.userId||sid,userId:p.userId||p.studentId||sid,studentName:name(p),email:p.email||'',kurs:c,kursnummer:c,courseCode:c,rankingPoints:pts,pointsTotal:pts,rankingMirrorVersion:1,rankingUpdatedAt:serverTimestamp()},{merge:true});return true}catch(e){console.warn('Ranglisten-Mirror fehlgeschlagen',e);return false}}
  function patch(){const api=window.SPProgress;if(!api||api.__rankingMirrorV1)return false;for(const method of ['recordTaskProgress','recordExamResult']){const raw=api[method]?.bind(api);if(typeof raw!=='function')continue;api[method]=async function(payload={}){const result=await raw(payload);const pts=totalFrom(result||{});if(result&&pts>0)mirror(pts);return result}}
    api.__rankingMirrorV1=true;window.SPProgress=api;return true}

  ...
  for(const row of docs||[]){
    for(const[key,topic]of Object.entries(row.data?.wortschatz||{})){
      ...
      for(const[file,task]of Object.entries(topic.tasks||{})){
        const state=stateFromTask(task);if(!state)continue;const raw=JSON.stringify(state);
        for(const localKey of localKeysFor(nums.lesson,nums.theme,file)){
          const old=localStorage.getItem(localKey);if(old===null||core.strength(raw)>core.strength(old)){safeSet(localKey,raw);restored++}
        }
      }
    }
  }
...
function strongest(remoteEntry,localValue){
  if(localValue==null)return{source:'cloud',value:remoteEntry?.value??null};
  if(!remoteEntry)return{source:'local',value:String(localValue)};
  return core.strength(localValue)>=core.strength(remoteEntry.value)?{source:'local',value:String(localValue)}:{source:'cloud',value:String(remoteEntry.value)};
}
async function reconcileSnapshot(remote,{writeBack=true}={}){
  ...
  let rescuedLocal=0,cloudWon=0;
  for(const[key,value]of local){
    const choice=strongest(merged.get(key),value);
    if(choice.source==='local'){
      const old=merged.get(key);if(!old||String(old.value)!==String(value)){rescuedLocal++;journal.entries[key]={value:String(value),updatedAt:Date.now()}}
      merged.set(key,{key,value:String(value),deleted:false,updatedAt:Date.now()});
    }else cloudWon++;
  }
  ...
  if(writeBack&&(rescuedLocal>0||!remote.authorityReady)){
    await setDoc(doc(db,'progress',activeId),{
      ...
      clientProgressAuthorityVersion:AUTHORITY_VERSION,clientProgressAuthorityMode:'non-destructive-merge-v3',clientProgressAuthorityActivatedAt:serverTimestamp()
    },{merge:true});
  }
  return{removed:0,pendingApplied:rescuedLocal,cloudWon,restored:merged.size,restoredStructured,rescuedLocal};
}
    ...
    try{
      ...
      await runTransaction(db,async transaction=>{
        ...
        for(const key of keys){
          const pending=pendingEntries[key];if(!pending)continue;const remote=entries.get(key);
          if(!remote||core.strength(pending.value)>=core.strength(remote.value))entries.set(key,{key,value:String(pending.value),deleted:false,updatedAt:Number(pending.updatedAt)||Date.now()});
        }
        const map=core.buildMap(entries);
        transaction.set(ref,{[FIELD]:map,clientProgressStateVersion:core.STATE_VERSION,clientProgressStateUpdatedAt:serverTimestamp(),clientProgressNonDestructive:true,clientProgressAuthorityVersion:AUTHORITY_VERSION,clientProgressAuthorityMode:'non-destructive-merge-v3'},{merge:true});
      ...
      try{window.dispatchEvent(new CustomEvent('SP_ACCOUNT_PROGRESS_SYNCED',{detail:{studentId:activeId,keys:keys.length,serverAuthoritative:false,nonDestructive:true,authorityVersion:AUTHORITY_VERSION}}))}catch(e){}
      return{ok:true,keys:keys.length,nonDestructive:true};
...
async function refreshFromCloud(){
  ...
  refreshPromise=(async()=>{
    try{const remote=await readServerRemote(),applied=await reconcileSnapshot(remote,{writeBack:true});if(dirty.size)await flush();try{window.dispatchEvent(new CustomEvent('SP_ACCOUNT_PROGRESS_REFRESHED',{detail:{studentId:activeId,nonDestructive:true,authorityVersion:AUTHORITY_VERSION,...applied}}))}catch(e){}return{ok:true,...applied}}
...
export async function startAccountProgressSync(){
  ...
  try{
    ...
    const result={active:true,studentId:activeId,authorityMode:'non-destructive-merge-v3',authorityVersion:AUTHORITY_VERSION,serverAuthoritative:false,nonDestructive:true,source:'firestore-server-plus-local',sourceDocs:remote.docs.length,authorityRepaired:!remote.authorityReady,...applied};
...
window.SPAccountProgressSync={start:startAccountProgressSync,flush,refresh:refreshFromCloud,ready:accountProgressReady,field:FIELD,version:core.STATE_VERSION,authorityVersion:AUTHORITY_VERSION,serverAuthoritative:false,nonDestructive:true};
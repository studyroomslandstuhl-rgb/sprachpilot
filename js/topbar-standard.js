...
function isWortschatzPage(){return /\/wortschatz\/A\d-Lektion-\d+\//.test(location.pathname)}
...
function navHtml(items){const all=[{type:'a',text:'← Zurück',href:backHref()},...items];return all.map(item=>item.type==='button'?'<button class="btn secondary" type="button" onclick="'+safeText(item.onclick||'')+'">'+safeText(item.text)+'</button>':'<a class="btn secondary" href="'+safeText(item.href||'#')+'">'+safeText(item.text)+'</a>').join('')}
function findLegacyHeader(){return document.querySelector('header.sp-shell, header:has(.sp-head), header:has(.sp-brand), header:has(.teacher-top)')}
function ensureHeader(){let legacy=findLegacyHeader();let standard=document.querySelector('header.topbar[data-sp-standard-topbar="1"], header.topbar');if(legacy&&!legacy.classList.contains('topbar')){legacy.classList.add('topbar');return legacy}if(standard)return standard;const container=document.querySelector('.container')||document.querySelector('.page')||document.body;const header=document.createElement('header');header.className='topbar';container.insertBefore(header,container.firstChild);return header}
function removeLegacyHeaders(keep){document.querySelectorAll('header.sp-shell, header').forEach(h=>{if(h===keep)return;if(h.dataset.spStandardTopbar==='1')return;if(h.classList.contains('sp-shell')||h.querySelector('.sp-head')||h.querySelector('.sp-brand'))h.remove()})}
function renderTopbar(){injectTopbarCss();if(document.body.classList.contains('teacher-dashboard'))return;const header=ensureHeader();const info=pageInfo(header);const navItems=existingNavLinks(header);const fallback=pageNavFallback();const labels=new Set(navItems.map(x=>x.text));fallback.forEach(x=>{if(!labels.has(x.text))navItems.push(x)});header.className='topbar';header.innerHTML='<div class="topbar-main sp-account-row"><a class="brand" href="/index.html"><div class="logo sp-standard-logo"><img src="/assets/logo/sprachpilot-logo.png" alt="SprachPilot"></div><div><h1>'+safeText(info.title)+'</h1><div class="subtitle">'+safeText(info.subtitle)+'</div></div></a><div class="account-tools"><span class="account-pill">'+safeText(profileName())+'</span><a class="account-link" href="'+safeText(dashboardHref())+'">Dashboard</a><a class="account-link" href="/profile/index.html">Profil</a><button class="account-link account-btn" type="button" id="spGlobalLogout">Abmelden</button></div></div><nav class="nav sp-page-nav">'+navHtml(navItems)+'</nav>';header.dataset.spStandardTopbar='1';const btn=header.querySelector('#spGlobalLogout');if(btn)btn.addEventListener('click',logout);removeLegacyHeaders(header);removeDuplicateBackButtons()}

...
function pageInfo(header){const oldTitle=header?.querySelector('h1')?.textContent?.trim()||'SprachPilot';const oldSub=header?.querySelector('.subtitle,.sp-subtitle,.sub')?.textContent?.trim()||document.title||'';return {title:oldTitle,subtitle:oldSub}}
...
function backHref(){if(isDashboardPage())return '/index.html';if(isTaskPage())return 'index.html';if(isThemePage())return '../index.html';if(isWortschatzLessonPage())return '../index.html';if(isVerbenPage()||isFragenPage())return '/index.html';return 'javascript:history.back()'}
...
function ensureHeader(){let legacy=findLegacyHeader();if(legacy){legacy.classList.add('topbar');return legacy}const container=document.querySelector('.container')||document.querySelector('.wrap')||document.querySelector('.page')||document.body;const header=document.createElement('header');header.className='topbar';container.insertBefore(header,container.firstChild);return header}

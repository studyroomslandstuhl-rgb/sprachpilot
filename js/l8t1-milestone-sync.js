...
function ownerCandidates(){const p=profile();return [...new Set([p.authUid,p.canonicalStudentId,p.docId,p.studentId,p.userId,p.uid,p.id,localStorage.getItem('SP_STUDENT_ID'),p.email].map(clean).filter(Boolean))]}
...
async function doFlush(reason='auto'){
 ...
 const originalRun=localStorage.getItem(RUN_KEY);let synced=0,failed=0;
 try{
  for(const [runRaw,runData] of Object.entries(student.runs||{})){
   ...
   for(const [id,st] of Object.entries(runData?.tasks||{})){
    ...
    const key=`R${run}:${id}`,sig=signature(run,id,st);if(acked(readMaster(),owner,key,sig))continue;
    try{
 ...
 try{window.dispatchEvent(new CustomEvent('SP_L8T1_MILESTONES_SYNCED',{detail:{reason,synced,failed}}))}catch(e){}
 return{ok:failed===0,reason,synced,failed};
}

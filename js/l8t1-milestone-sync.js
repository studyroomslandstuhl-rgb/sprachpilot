...
async function api(){try{await normalizeIdentity();if(!window.SPProgress?.recordTaskProgress)await import('/js/progress.js?v=20260831-central2');return window.SPProgress?.recordTaskProgress?window.SPProgress:null}catch(error){console.warn('L8T1 milestone sync: Fortschritts-API fehlt',error);return null}}
async function doFlush(reason='auto'){
 ...
 const initialServerRun=cloudRun(serverProgress);let synced=0,failed=0,eligible=0,syncedHistorical=0;const originalRun=localStorage.getItem(RUN_KEY);
 try{
  ...
  for(const [runRaw,runData] of runs){
   ...
   for(const [id,st] of Object.entries(runData?.tasks||{})){
    ...
    try{
     localStorage.setItem(RUN_KEY,String(run));const total=Math.max(1,Number(st.total)||1),done=doneCount(st),correct=Math.max(0,Math.min(total,Number(st.firstCorrect)||0)),percent=clamp(correct/total*100),result=isExam(id)
       ?await P.recordExamResult({module:'wortschatz',moduleTitle:'Wortschatz',level:'A1',lesson:8,theme:1,topicId:TOPIC,title:'A1 Lektion 8 · Thema 1',file:'pruefung.html',run,score:correct,maxScore:total,percent,scorePercent:percent,stars:percent>=100?3:percent>=70?2:percent>=50?1:0})
       :await P.recordTaskProgress({module:'wortschatz',moduleTitle:'Wortschatz',level:'A1',lesson:8,theme:1,topicId:TOPIC,title:'A1 Lektion 8 · Thema 1',file:`task.html?task=${id}`,taskKey:id,taskTitle:taskTitle(id),run,total,done,percent:100,completed:true,countAttempt:false});
     if(result==null)throw new Error('Zentraler Fortschrittsschreibvorgang nicht bestätigt');saveAck(owner,key,sig);synced++;if(run<initialServerRun)syncedHistorical++;
 ...
 try{window.dispatchEvent(new CustomEvent('SP_L8T1_MILESTONES_SYNCED',{detail:{reason,synced,failed,eligible,syncedHistorical,centralWriter:true,explicitRuns:true}}))}catch(e){}
 return{ok:failed===0,reason,synced,failed,eligible,syncedHistorical,centralWriter:true,explicitRuns:true};
}

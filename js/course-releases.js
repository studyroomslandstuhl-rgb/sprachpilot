...
export function themeOpen(data,module,lessonKey,themeKey){if(isTeacher())return true;if(!hasReleaseData(data))return false;const m=normalizeModule(module);if(moduleExplicitOff(data,m))return false;const keys=[themeKey,`${lessonKey}/${themeKey}`,...m.aliases.map(a=>`${a}/${lessonKey}/${themeKey}`)],vals=keys.map(k=>data.enabledThemes?.[k]);m.aliases.forEach(a=>vals.push(getPath(data,["releases",a,"lessons",lessonKey,"themes",themeKey,"enabled"])));if(vals.some(v=>v===true))return true;if(vals.some(v=>v===false))return false;return defaultOpen(data)}
export function taskOpen(data,module,lessonKey,themeKey,file){if(isTeacher())return true;if(!hasReleaseData(data))return false;return themeOpen(data,module,lessonKey,themeKey)}

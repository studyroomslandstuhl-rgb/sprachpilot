import { recordTaskProgress, recordExamResult, recordThemeReset } from "/js/progress.js?v=10";
...
async function awardTask(file,options={}){if(isExamFile(file)){const p=options.payload||{};const percent=clamp(p.percent??p.scorePercent??p.score??100);return awardExam({percent},options)}return recordTaskProgress({...taskPayload(file,100),...(options.payload||{})})}
async function awardExam(result={},options={}){const percent=clamp(result.percent??result.scorePercent??result.score??100);const p={...taskPayload('pruefung.html',percent),scorePercent:percent,score:percent,stars:percent>=100?3:percent>=70?2:percent>=50?1:0,...(options.payload||{})};return recordExamResult(p)}
...
function patch(){
  if(window.__SP_SCORING_PATCHED_V10)return;window.__SP_SCORING_PATCHED_V10=true;
  const later=()=>{
    if(typeof window.complete==="function"&&!window.complete.__spScoringV10){const old=window.complete;window.complete=function(area,file,nextFile){const out=old.apply(this,arguments);if(isExamFile(file))awardExam({percent:100});else awardTask(file);return out};window.complete.__spScoringV10=true;}
    if(typeof window.done==="function"&&!window.done.__spScoringV10){const old=window.done;window.done=function(file,total){const out=old.apply(this,arguments);awardTask(file,{payload:{total:Number(total||100),done:Number(total||100)}});return out};window.done.__spScoringV10=true;}
    if(typeof window.finishTask==="function"&&!window.finishTask.__spScoringV10){const old=window.finishTask;window.finishTask=function(file){const out=old.apply(this,arguments);if(isExamFile(file))awardExam({percent:100});else awardTask(file);return out};window.finishTask.__spScoringV10=true;}
    if(typeof window.saveTask==="function"&&!window.saveTask.__spScoringV10){const old=window.saveTask;window.saveTask=function(file,st){const out=old.apply(this,arguments);const percent=statePercent(st||{});if(isExamFile(file))awardExam({percent});else if(percent>=100)awardTask(file,{payload:{total:Number(st?.total||100),done:Number(st?.done?.length||st?.done||100)}});return out};window.saveTask.__spScoringV10=true;}
    if(typeof window.saveExamResult==="function"&&!window.saveExamResult.__spScoringV10){const old=window.saveExamResult;window.saveExamResult=function(result){const out=old.apply(this,arguments);awardExam(result||{});return out};window.saveExamResult.__spScoringV10=true;}
    if(typeof window.syncExam==="function"&&!window.syncExam.__spScoringV10){const old=window.syncExam;window.syncExam=function(result){const out=old.apply(this,arguments);awardExam(result||{});return out};window.syncExam.__spScoringV10=true;}
    drainQueues();
  };
  later();document.addEventListener("DOMContentLoaded",later);setTimeout(later,300);setTimeout(later,1200);
}

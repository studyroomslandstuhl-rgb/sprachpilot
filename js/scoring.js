...
function patch(){
  ...
  const later=()=>{
    if(typeof window.complete==="function"&&!window.complete.__spScoringV10){const old=window.complete;window.complete=function(area,file,nextFile){const out=old.apply(this,arguments);if(String(file||"").includes("pruefung"))awardExam({percent:100});else awardTask(file);return out};window.complete.__spScoringV10=true;}
    ...
    if(typeof window.finishTask==="function"&&!window.finishTask.__spScoringV10){const old=window.finishTask;window.finishTask=function(file){const out=old.apply(this,arguments);if(String(file||"").includes("pruefung"))awardExam({percent:100});else awardTask(file);return out};window.finishTask.__spScoringV10=true;}
    if(typeof window.saveTask==="function"&&!window.saveTask.__spScoringV10){const old=window.saveTask;window.saveTask=function(file,st){const out=old.apply(this,arguments);const percent=statePercent(st||{});if(String(file||"").includes("pruefung"))awardExam({percent});else if(percent>=100)awardTask(file,{payload:{total:Number(st?.total||100),done:Number(st?.done?.length||st?.done||100)}});return out};window.saveTask.__spScoringV10=true;}

...
async function render(config={}){
 ...
 const row=item=>{const word=term(item),image=String(item?.image||''),tr=String((config.translationOf?config.translationOf(item):item?.translation)||'');return `<article class="sp-vocab-word"><div class="sp-vocab-image">${image?`<img src="${esc(image)}" alt="" aria-hidden="true" loading="lazy" onerror="this.remove()">`:''}</div><div class="sp-vocab-content"><h3>${esc(word)}</h3>${detail(item)}<div class="sp-vocab-translation"><b>${esc(translationLabel)}:</b> ${tr?esc(tr):'<span>–</span>'}</div></div><button class="sp-vocab-audio" type="button" data-sp-vocab-audio="${item.__spOverviewIndex}" aria-label="${esc(word)} anhören">🔊 <span>Hören</span></button></article>`};

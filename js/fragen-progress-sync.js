...
function findFragenState(){
  const active=localStorage.getItem("A1_ACTIVE_SESSION");
  if(active){const s=read(active);if(s&&s.exerciseProgress)return s}
  for(let i=0;i<localStorage.length;i++){
    const k=localStorage.key(i);
    if(!k||!k.startsWith("A1_")||k==="A1_ACTIVE_SESSION"||k.startsWith("A1_STUDENTS_"))continue;
    const s=read(k);if(s&&s.exerciseProgress)return s;
  }
  return null;
}
...
function schedule(){clearTimeout(timer);timer=setTimeout(()=>syncFragen().catch(e=>console.warn("Fragen Sync fehlgeschlagen",e)),700)}
...
patch();schedule();setTimeout(schedule,1400);window.addEventListener("online",schedule);window.addEventListener("pagehide",()=>{try{syncFragen()}catch(e){}});window.SPFragenProgressSync={sync:syncFragen};

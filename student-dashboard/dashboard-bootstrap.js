...
function alreadyCanonicalSecureProfile(profile,uid){
  ...
  return !!(canonical&&p.secureAuth===true&&String(p.authUid||'')===String(uid||'')&&String(p.docId||canonical)===canonical&&String(p.studentId||canonical)===canonical&&String(p.userId||canonical)===canonical);
}

(function(){
'use strict';
if(window.__SP_DASHBOARD_POINTS_SELF_HEAL_V2)return;window.__SP_DASHBOARD_POINTS_SELF_HEAL_V2=true;
let timer=null,running=false,last=-1,serverRefreshRunning=false;
...
window.SP_DASHBOARD_DATIV_POINTS_READY=(async()=>{
 try{
  await import('/shared/points-recalculator.js?v=20260829-dativ3');
  await import('/shared/dativ-points-extension.js?v=3');
  for(let i=0;i<80;i++){
...
async function refreshExactPoints(){
 ...
 try{
  ...
  if(!id)return false;
  const snap=await getDocFromServer(doc(db,'progress',id));if(!snap.exists())return false;
  ...
  const total=Math.max(calculated,stored);
...
const start=()=>{
 ...
 [150,450,900,1800,4500,10000].forEach(delay=>setTimeout(()=>reconcile(),delay));
};

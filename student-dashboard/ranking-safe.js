...
function realStudent(x){
  const name=String(displayName(x)||'').trim();
  const role=String(x.role||x.loginRole||'').toLowerCase();
  if(role==='teacher'||x.isTeacher===true||x.teacherPreview===true)return false;
  if(!name&&!x.email)return false;
  if(/^sch(ü|ue)ler\/?in$/i.test(name)||/^student$/i.test(name))return false;
  return hasActivity(x);
}
...
async function loadRankingSafe(){
  ...
  box.innerHTML='<div class="empty">Rangliste lädt …</div>';
  const c=courseCode();
  if(!c){box.innerHTML='<div class="empty">Rangliste: Kurs noch nicht erkannt.</div>';return}
  const rows=[
    ...(await queryRows('kurs',c)),
    ...(await queryRows('courseCode',c)),
    ...(await queryRows('kursnummer',c))
  ];
  const seen=new Map();
  rows.forEach(x=>{if(!seen.has(x.id))seen.set(x.id,x)});
...
if(btn)btn.addEventListener('click',loadRankingSafe);
setTimeout(loadRankingSafe,200);
setTimeout(loadRankingSafe,1200);
window.addEventListener('SP_PROFILE_SYNCED',loadRankingSafe);
window.addEventListener('SP_PROGRESS_SYNCED',loadRankingSafe);

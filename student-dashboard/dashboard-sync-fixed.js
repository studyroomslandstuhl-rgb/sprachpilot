...
let progressDoc={},statsBusy=false,rankingBusy=false,refreshTimer=null;
...
function mergeTask(a={},b={}){const pa=taskPct(a),pb=taskPct(b),strong=pa>=pb?a:b,weak=strong===a?b:a;return{...weak,...strong,percent:Math.max(pa,pb),completed:!!(a.completed||b.completed||pa>=100||pb>=100),done:Math.max(Number(a.done||0),Number(b.done||0)),total:Math.max(Number(a.total||0),Number(b.total||0)),points:Math.max(Number(a.points||0),Number(b.points||0)),pointsByRun:{...(a.pointsByRun||{}),...(b.pointsByRun||{})}}}
...
async function readOwnProgress(){
 const p=profile(),jobs=progressIds(p).map(async id=>{try{const s=await getDoc(doc(db,'progress',id));return s.exists()?s.data()||{}:null}catch(e){return null}});
 const mail=String(p.email||'').trim().toLowerCase();
 if(mail)jobs.push((async()=>{try{const s=await getDocs(query(collection(db,'progress'),where('email','==',mail),limit(20)));let merged={};for(const d of s.docs)merged=mergeProgress(d.data()||{},merged);return merged}catch(e){return null}})());
 const rows=await withTimeout(Promise.all(jobs),8000,'Eigener Fortschritt lädt zu lange');
 let merged={};for(const row of rows)if(row)merged=mergeProgress(row,merged);return merged;
}
...
function renderStats(data,save=true){const p=profile();$('userPill').textContent=userName(p)+(courseCode(p)?' · '+courseCode(p):'');$('totalCircle').style.setProperty('--p',data.avg||0);$('totalCircle').innerHTML='<span>'+Number(data.avg||0)+'%</span>';$('totalFill').style.width=Number(data.avg||0)+'%';$('summaryText').textContent=data.totalThemes?`${data.done} von ${data.totalThemes} Themen fertig`:'Noch kein Themenfortschritt gespeichert.';$('pointsTotal').textContent=Number(data.points||0);$('starsTotal').textContent=Number(data.stars||0);$('doneTotal').textContent=Number(data.done||0);$('lastUpdated').textContent='Aktualisiert: '+new Date().toLocaleTimeString('de-DE',{hour:'2-digit',minute:'2-digit'});if(save){try{localStorage.setItem(DASH_CACHE,JSON.stringify(data));localStorage.setItem('SP_POINTS_TOTAL',String(data.points||0))}catch(e){}}}
async function refreshStats(){if(statsBusy)return;statsBusy=true;try{const r=await readOwnProgress();if(r&&Object.keys(r).length)progressDoc=r;renderStats(statsFrom(progressDoc))}catch(error){console.warn('Dashboard-Lernstand konnte nicht aktualisiert werden',error);const cached=readJSON(DASH_CACHE,null);if(cached)renderStats(cached,false);$('lastUpdated').textContent='Letzter gespeicherter Stand – Verbindung wird erneut versucht.'}finally{statsBusy=false}}
...
const cached=readJSON(DASH_CACHE,null);if(cached)renderStats(cached,false);else renderStats({avg:0,done:0,totalThemes:0,points:point(localStorage.getItem('SP_POINTS_TOTAL')),stars:0},false);
